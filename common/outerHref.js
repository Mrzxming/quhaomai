...
export default async function outerHref(url, built, mobile) {
	console.log("===================")
	// console.log(123, url)
	// console.log(456, built)
	// console.log(789, mobile)
	if (mobile === '') {
		uni.showModal({
	...
	if (built == 'app') {
		let reg = RegExp(/index\/index|middles\/middles|cart\/cart|user\/user|integration\/integration/);
	...
	if (built != undefined && built != 'undefined' && built != 'onback') {
		...
		let roles = await store.dispatch('setShopConfig', {
			type: true
		});
		uni.setStorage({
			key: 'configData',
