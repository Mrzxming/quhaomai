...
export default {
	...
	computed: {
		weappInReview() {
			return uni.getStorageSync('configData') && uni.getStorageSync('configData').weapp_in_review ? uni
				.getStorageSync('configData').weapp_in_review : ''
		},
		appInReview() {
			return uni.getStorageSync('configData') && uni.getStorageSync('configData').app_in_review ? uni
				.getStorageSync('configData').app_in_review : ''
		},
		imagePath() {
		...
		getRegionData() {
			return uni.getStorageSync("regionData") ? uni.getStorageSync("regionData") : (uni.getStorageSync(
				"userRegion") ? uni.getStorageSync("userRegion") : this.regionData);
		},
		//是否是审核模式
		controlVersion() {
			let fald = false
	...
	methods: {
		...
		//客服
		onChat(shop_name, goods_id, shop_id) {
			if (this.$isLogin()) {
				let type = ''
				// #ifndef MP-WEIXIN
				type = 'app'
					...
					if (res.status == 'success') {
						...
						if (url) {
							...
							if (isUrl && platform == 'ios') {
								...
								plus.runtime.openURL('mqq://im/chat?chat_type=wpa&uin=' + qq +
							...
							uni.navigateTo({
								url: "/pagesB/kefu_center/kefu_center?shop_id=" + res.data.ru_id +
									'&shop_name=' + shop_name + '&goods_id=' + res.data.goods_id +
									'&token=' + res.data.t
								// url:"/pagesC/kefu/kefu?shop_id=" + res.data.ru_id + '&shop_name=' + shop_name + '&goods_id=' + res.data.goods_id + '&token='+ res.data.t
							})
						}
					} else {
						uni.showToast({
		...
		shareInfo(info, poster, eid) {
			console.log('unishareInfo',info, poster, eid)
			if (eid) {
				this.eid = eid
			}
			let shareInfo = info
			...
			let shareList = [{
					icon: "https://images.eqingqu.com/attachs/photo/202404/20240416_F73554502AC82BF20FA8F7F3076747E8.png",
					text: "微信好友",
					provider: 'weixin'
				},
				{
					icon: "https://images.eqingqu.com/attachs/photo/202404/20240416_809F751F337086C51E943ECBBBBDD4D6.png",
					text: "朋友圈",
					provider: 'pengyouquan'
				},
				{
					icon: "https://images.eqingqu.com/attachs/photo/202404/20240416_5A8A468CABCEB6602D82C3C0CEFF177D.png",
					text: "复制",
					provider: 'copy'
				},
				// {
				// 	icon: "https://images.eqingqu.com/attachs/photo/202404/20240416_B6213E2A3A355C11CB0E3FA43DA2658A.png",
				// 	text: "更多",
				// 	provider: 'more'
				// },
			];
			...
			if (poster) {
				shareList.splice(2, 0, {
					icon: "https://images.eqingqu.com/attachs/photo/202404/20240416_E966F5B8FE131269A6DFDE7C013C1E64.png",
					text: "生成海报",
					provider: 'poster'
				})
			}
			...
			uni.getProvider({
				...
				success: (res) => {
					this.shareObj = share(shareInfo, shareList, function(index) {
						...
						if (str && (str == 'pengyouquan' || str == 'weixin')) {
							that.from = 'wechat'
							if (that.eid && that.from) {
								uni.request({
									url: 'https://www.ok9288.com/api/drpnew/share_img_url',
									method: 'GET',
									data: {
										id: that.eid,
										share_platform: that.from
									},
									header: {
