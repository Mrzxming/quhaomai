template
	view class=uni-page-body middles-page class={'middles-has-top-bg' !!headerBgImage} style=background-color #ffffff;
		!-- 顶部区域包装：仅 search + 分类 Tab；金刚区在下方 scroll-view 内，下拉刷新从金刚区顶缘开始 --
		view class=header-with-bg class={'has-header-bg' !!headerBgImage, 'header-scrolled' navOpacity  0.12}
			!-- 背景层：上红下白渐变  或图片(PNGGIF)，后续由后端返回 headerBgImage --
			view class=header-bg-layer v-if=headerBgImage
				view v-if=headerBgImage === 'gradient' class=header-bg-gradientview
		...
		view class=search_container
		  ...
		  !-- #ifdef H5 --
		  !-- app-down configData=shopConfig @localShow=localShow v-if=shopConfig && shopConfig.wap_index_pro == 1app-down --
		  div class='app-down' v-if=localShow
			  div class=ect-header-banner dis-box id=ect-banner
				  ...
				  view style=padding-top 34rpx;padding-left 20rpx; @click=closeAppDown
					  image
						  src=httpsimages.eqingqu.comattachsphoto20240320240305_A596460B29B83F980245FE085E084168.png
						  style=width 40rpx;height 40rpx;image
				  view
				  div class=box-flex
					  img src=shopConfig.h5_index_pro_image'httpsimages.eqingqu.comattachsphoto20250520250509_306AA5BD7E90958E102A5CE61A5352D2.png'
		  ...
		  !-- view class=search class={'position-fixed'scrollFixed}
			...
			view class=search class={'position-fixed'scrollFixed} style=searchHeaderStyle
			  ...
			  view class=status_barview

			  !-- #endif --
			  !-- #ifdef MP-WEIXIN --
			  view class=status_bar style={heightmenuButtonInfo.top + 'px'}view
			  ...
			  view class=input_main  style=searchInputStyle
				  view class=input_view
					  view style=align-items center;display flex;flex-direction row;
						  !-- #ifdef H5 --
						  image
							  src=httpsimages.eqingqu.comattachsphoto20240420240416_9028623ECE1EDAA78E8FFE7BA3FDB47B.png
					  ...
					  view style=align-items center;display flex;flex-direction row;
						  !-- #ifdef H5 --
						  image src=pagesCstaticcamera@2x.png style=width 44rpx;height 40rpx;
						  image
						  !-- #endif --
						  !-- #ifdef APP-PLUS --
						  image src=pagesCstaticcamera@2x.png style=width 44rpx;height 40rpx;
						  ...
						  !-- #endif --
						  !-- #ifdef MP-WEIXIN --
						  image src=@pagesCstaticcamera@2x.png style=width 36rpx;height 36rpx;
							  @click=chooseImage
						  image
						  !-- #endif --
						  !-- view
			  ...
			  view class=search_all_categories
				  ...
				  view class=scroll-view style=width 97%;
					  view class=tab-bar1
						  ...
						  view
						  view class=all-text
							  text @click=tapAllCategories()全部分类text
							  text class=iconfont icon-more size_24text
						  view
			  ...
			  view class=all_categories_popops v-show=allCategoriesType
				  ...
				  !-- #endif --
				  !-- #ifdef MP-WEIXIN --
				  view class=all_categories_label style={toptopVal + 'px'}
					  view class=all_categories_label_top @click=tapAllCategories()
						  text全部分类text
						  text class=iconfont icon-more size_24text
					  view
		  ...
		  !-- #endif --
		  !-- #ifdef MP-WEIXIN --
		  view style={heightnavLabelTop + 'px'}view
		  !-- view style=height 300upx;view --
		  !-- #endif --
		view
		view

	  !-- 方案 A：仅顶部 search+分类 固定；金刚区起进入 scroll-view，下拉刷新从金刚区上方露出（类 1688） --
	  scroll-view
		  class=middles-main-scroll
		  scroll-y
		  enable-back-to-top
		  ...
		  view slot=refresher class=middles-refresher-slot
			  view class=middles-refresher-inner class={'middles-refresher-loading' refresherTriggered}
				  view v-if=refresherTriggered class=middles-refresher-spinnerview
				  text v-else class=middles-refresher-mark{{ refresherMarkText }}text
				  text class=middles-refresher-hint{{ refresherHintText }}text
			  view
		  view
	  ...
	  view class=nav  v-if=false  class=aClass
		  swiper style=height 200rpx; indicator-dots=false @change=swiperChange
			  !-- swiper class=swiper indicator-dots=indicatorDots style={heightimgheightsnav}
			  @change=swiperChange --
	  ...
	  view class=new-person v-if=receiveData.length  0 @click=goNewPerson()
		  view class=one
			  ...
			  view class=one-text
				  ...
				  text class=mr8 后过期text
			  view
			  view class=one-more
				  image
					  src=httpsimages.eqingqu.comattachsphoto20250820250828_F29BA621659B4561BA4DDF590402E3F1.png
				  image
			  view
		  view
		  view class=two
			  view class=two-item v-for=(item,index) in receiveData.slice(0, 4) key='index'
				  view class=two-content
					  ...
					  text class=two-condition满{{item.cou_man}}可用text
				  view
			  view
			  view class=two-text
				  text class=two-text-line立即text
				  text class=two-text-line领取text
			  view
		  view
		  view class=three
			  view class=three-item v-for=(item,idx) in userGoodsList.slice(0,3) key=idx
				  view class=three-content
					  ...
					  view class=img-box
						  image class=img src=item.goods_thumb mode=widthFiximage

					  view
					  view class=info-box
						  view class=goods-name twolist-hidden  title-font style=font-size28rpx;
							  @longpress=copyTitle(item.title,item.goods_name)

							  image class=new-img
								  src=httpsimages.eqingqu.comattachsphoto20250820250829_38B6A8B4CCFFEEA55C9363D0A017ED2C.png
							  image
						  ...
						  view class=num-line
							  currency-price class=left-num price=item.shop_price size=20currency-price
							  view
								  text
	  ...
	  view v-if=false style=margin-top 20rpx;
		  ...
		  dsc-count-down v-if=countDown.module == 'count-down' && countDown.isShow module=countDown.data
			  preview=false shopId=ru_iddsc-count-down
	  view
	  ...
	  view class=visual-adv class=[visualAdvIsCarousel  'visual-adv-swiper'  'visual-adv-lie']
				  ...
				  swiper class=visual-adv-banner-swiper style={ height visualAdvSwiperHeightComputed + 'rpx' }
					  ...
					  view class=bg-img @click=linkvisualadv(items.data.allValue)
						  image src=items.data.allValue.titleImg mode=widthFix class=img
							  v-if=items.data.allValue.titleImg @load=visualAdvImageLoadimage
						  ...
						  scroll-view class=scroll-view scroll-x=true scroll-left=0
							  view class=scroll-view-item v-for=(item, index) in items.previewProlist key=index
								  ...
								  view class=goods-top
									  image src=item.goods_thumb class=img mode=widthFiximage
								  ...
								  view class=goods-info
									  ...
									  view
										  ...
										  image
											  src=httpsshhai2.oss-cn-shanghai.aliyuncs.comdatagallery_album5original_img5_P_1705973605849.png
		  ...
		  block v-else
			  block v-for=(items, indexs) in visualdvAList key=indexs
				  view class=bg-img @click=linkvisualadv(items.data.allValue)
					  image src=items.data.allValue.titleImg mode=widthFix class=img
						  v-if=items.data.allValue.titleImgimage
				  view
	  ...
	  view class=waterFall
		  ...
		  view
			  style= display flex;justify-content space-between;margin16rpx 16rpx 16rpx 16rpx;margin-right 16px;

			  view class=shipin_1 style={
  backgroundImage shipinswiperCurrent === 0 
	 'url(httpsimages.eqingqu.comattachsphoto20250320250331_C1DECA47A0B6E454FD79CA6F7D284A51.png)' 
	 'url(httpsimages.eqingqu.comattachsphoto20250420250403_5C0753A53388BF31560A7396FFFFD13F.png)'
}
							  ...
							  view class=new_goods_item v-for=(goodsItem,goodsidx) in items key=goodsidx
								  ...
								  view
									  image src=goodsItem.goods_thumb
										  ...
										  v-if=goodsItem.goods_thumbimage
									  !-- image style=width 146rpx;height 146rpx;border-radius 10rpx;max-width 146rpx;max-height 146rpx; src=httpswww.ok9288.comimguser_default.pngimage --
								  view
								  view class=newPrice
									  ￥
									  span style=font-size 28rpx;
										  v-if=goodsItem.shop_price{{goodsItem.shop_price}} span
									  !-- {{newListOne.shop_price}} --
								  view
							  view
						  view
					  swiper-item
			  ...
			  view class=shipin_2 style=
				  ...
				  !-- view class=jiangbei @click=goRankingList
					  style={backgroundImage 'url(httpsimages.eqingqu.comattachsphoto20250720250709_C6ACF45FA755B6AED201B1FD10FC5ADF.png)'}
				  view --
				  ...
				  view style=display flex;flex-direction column; class=shipin_2-label
						  ...
						  view class=shipin_2-left style=text-align center;width 50%;
							  ...
							  view
								  ...
								  image src=hotListOne && hotListOne.goods_thumb
									  ...
									  v-if=hotListOne && hotListOne.goods_thumbimage
								  image
									  src=httpsshhai2.oss-cn-shanghai.aliyuncs.comdatagallery_album5original_img5_P_1705973846451.jpg
									  class=image v-else
						  ...
						  view class=shipin_2-right style=text-align left;width 50%;
							  ...
							  view
								  ...
								  image src=hotListTwo && hotListTwo.goods_thumb
									  ...
									  v-if=hotListTwo && hotListTwo.goods_thumbimage
								  image
									  src=httpsshhai2.oss-cn-shanghai.aliyuncs.comdatagallery_album5original_img5_P_1705973846451.jpg
									  class=image v-else
									  style=max-width 146rpx;margin-top 8rpx;max-height 146rpx;image
							  ...
							  view class=newPrices
								  ￥
								  span style=font-size 28rpx;
									  v-if=hotListTwo && hotListTwo.shop_price{{hotListTwo.shop_price}} span
								  !-- {{hotListTwo.shop_price}} --
							  view
						  view
					  ...
					  view class=eightJin  v-if=shouldShowEightJin class={ 'fade-out' isClosing }
						  !-- view class=eightJin  v-if=shouldShowEightJin class={ 'fade-out' isClosing } --
						  text未满18岁禁止下载使用text
						  image @click=eightClose src=pagesCstaticwhiteClose.png class=imgEightimage
					  view
				  view
			  view
...
!--   view v-if=isDebug class=debug-center-region
		view class=debug-center-lineview
		view class=debug-observe-areaview
		view class=debug-center-text中心区域 - 仅在此切换GIFview
		view class=debug-threshold-areaview
	  view --
		  !-- 1 --
			  ...
			  view class=waterfall-wrapper
								...
								view style=position relative;
								  view
									view class=img-box style=height min-height 360rpx;
									  ...
									  view v-if=item.hasVideo class=gif-container id='gif-'+item.goods_id
										...
										view class=cover-content class={'cover-hidden' !showCover[item.goods_id]}
										  image class=cover-image src=item.goods_video_cover mode=aspectFill lazy-load=trueimage
										...
										!--  view v-if=isDebug 
											...
											text v-if=showingGif === item.goods_id ⭐中心text
										  view --
										view
										
										!-- GIF容器 --
										view class=gif-content v-if=!showCover[item.goods_id]
										  image class=goods-gif src=item.goods_video_gif mode=aspectFill lazy-load=trueimage
										view
									  view
					  
									  image v-else class=img src=item.goods_thumb mode=widthFix lazy-load=trueimage
									...
									view
										  ...
										  view class=goods-name twolist-hidden  title-font style=font-size14px;
											  @longpress=copyTitle(item.title,item.goods_name)
											  
											  !-- 热榜 --
											  view v-if=item.cat_rank_info class=top-one style=display inline-flex; margin-right 16rpx; vertical-align bottom;
												  view class=top-text
													  image src=pagesCstaticfly_left@2x.pngimage
													  textTOP{{item.cat_rank_info_cat_top}}text
													  image class=fly-right src=pagesCstaticfly_left@2x.pngimage
												  view
											  view
											  !-- 好卖严选 --
										  ...
										  view class=promo-tags-container
											  ...
											  view v-if=item.cou_name && item.cou_name.length  0
												  ...
												  image src=httpsimages.eqingqu.comattachsphoto20260120260129_FBDE6F91F8B9A667E5327B5C0FC27592.png 
											  ...
											  view v-if=item.first_j_price && item.first_j_price !== ''
												  style=display inline-flex; align-items center; font-weight 400;font-size 12px;color #FE2F02;
												  image src=httpsimages.eqingqu.comattachsphoto20260120260129_F6BCAA4FC4924C50925210754F44183A.png 
											  ...
											  view v-if=item.platm_min_price && item.platm_min_price !== ''
												  style=display inline-flex; align-items center; font-weight 400;font-size 12px;color #FE2F02;
												  image src=httpsimages.eqingqu.comattachsphoto20260120260130_EAA591B0B9E61CCE6678AE8DAA41143D.png 
				  ...
				  x-skeleton type=waterfall loading=loading configs=waterfallConfigs
					  ...
					  view class=column right-col
							  view class=right-placeholder rightview
					  ...
					  view style=position relative;
						  view
							  ...
							  view class=info-box
								  ...
								  view
								  view
									  style=display flex;flex-direction row;align-items center;font-size 22rpx;transformscale(0.8);margin-left -40rpx;width 120%;margin-top -10rpx;

									  view v-for=(items,index) in item.goods_services_label_new key=index
										  view
											  style=color #FE0302;border-radius 10rpx;border 2rpx solid #FE0302;transformscale(0.9);padding 0rpx 10rpx;margin-right 4rpx;
											  v-if=items =='黑卡'
											  {{items}}
										  view
										  view
								  ...
								  view
									  ...
									  text v-if=item.add_year style=color #A2A2A2;年text
	  ...
	  !--返回顶部--
	  !-- dsc-filter-top scrollState=scrollState outerClass=truedsc-filter-top --
	  ...
	  dsc-region display=regionShow isLevel=4 regionOptionData=regionData @updateDisplay=getRegionShow
		  @updateRegionDate=getRegionOptionDatedsc-region
	  !-- dsc-loading dscLoading=dscLoadingdsc-loading --
	  uni-popup show=bonusShow v-onhidePopup=handelClose('bonus')
		  view class=close @click=handelClose('bonus')
			  image
				  src=httpsimages.eqingqu.comattachsphoto20240420240416_53EB47F93542B07A5D951F4C4E187EA1.png
				  mode=widthFix class=imgimage
		  view
		  ...
		  view class=version-popup
			  ...
			  view class=close v-if=versionData.content.is_force_update==0 @click=handelClose('version')
				  image
					  ...
					  mode=widthFix class=imgimage
			  view
			  image src=imagePath.upgrade class=upgrade mode=widthFiximage
		  view
	  uni-popup
	  !-- uni-popup show=receiveStutas v-if=receiveStutas
		  view class=version-popup
			  ...
			  view class=close  @click=handelClose('receive')
				  image
					  src=pagesCstaticsearchcha.png
					  mode=widthFix class=imgimage
			  view
			  image src=httpsimages.eqingqu.comattachsphoto20250820250828_A2E771EA2F9C1A9F9A2725090831C9E7.png class=upgrade mode=widthFiximage
...
!-- 		uni-popup show=receiveStutas v-if=receiveStutas
		  view class=receive-popup
			  view class=receive-top
				  image
					  src=httpsimages.eqingqu.comattachsphoto20250820250828_D0BE827D3C1463B5688DF871911EC237.png
					  mode=widthFiximage
			  view
			  ...
			  view class=receive-list 
				  view class=receive-item v-for=(item,index) in receiveData.slice(0, 3) key='index'
					  view class=coupon-content
						  ...
						  text class=condition满{{item.cou_man}}可用text
					  view
				  view
				  view class=list-bottom-bgview
			  view


			  view class=close @click=handelClose('receive')
				  image src=pagesCstaticsearchcha.png mode=widthFix class=imgimage
			  view
			  view 
				  image
					  src=httpsimages.eqingqu.comattachsphoto20250820250828_C11BFC195C7B2C001A52811B586FCBF4.png
					  class=upgrade mode=widthFix  @click=goNewPersonimage
			  view
		 ...
		 view class=receive-popup
			 ...
			 block v-if=receiveData.length  0
				 view class=newuser-popup-wrap
					 view class=newuser-glow-ringview
					 view class=newuser-glow-ring newuser-glow-ring-2view
					 view class=newuser-glow-ring newuser-glow-ring-3view
					 view class=newuser-light-rays
						 view class=newuser-ray v-for=i in 6 key='ray'+iview
						view
					 view class=newuser-popup-box
						 view class=newuser-popup-header
							 view class=newuser-shine-effectview
							 view class=newuser-coins
								 view class=newuser-coin v-for=i in 6 key='coin'+i¥view
								view
							 text class=newuser-header-title新人限时专享text
							 text class=newuser-header-subtitle现在下单锁定优惠text
							view
						 view class=newuser-coupons-section
							 view class=newuser-coupons-grid
								 view class=newuser-coupon-card v-for=(item, index) in receiveData.slice(0, 3) key=index
									 view class=newuser-coupon-badge新人专享view
									 view class=newuser-coupon-amount
										 text class=newuser-coupon-currency¥text{{ item.cou_money }}text class=newuser-coupon-unit元text
						view
									 view class=newuser-coupon-condition满{{ item.cou_man }}可用view
									 view class=newuser-coupon-btn @click.stop=goNewPerson立即领取view
					view
								view
							 view class=newuser-timer-info
								 view class=newuser-countdown
									 text class=newuser-time-value{{ formatTime(newUserMinutes) }}text
									 text class=newuser-time-septext
									 text class=newuser-time-value{{ formatTime(newUserSeconds) }}text
									 text class=newuser-time-septext
									 text class=newuser-time-ms{{ newUserCentisecondsStr }}text
							view
								 text class=newuser-after-expire后失效text
								view
							view
						 view class=newuser-main-btn-wrap
							 view class=newuser-main-btn @click=goNewPerson立即领取view
						view
					view
					 view class=newuser-close-btn @click=handelClose('receive')
						 view class=newuser-close-btn-circleview
						 view class=newuser-close-btn-text{{ newUserCloseCountdown }}s 后关闭view
						 view class=newuser-close-icon×view
			view
				 view
			 block
			 !-- 店铺优惠券：仅当无新人数据、有店铺券数据且 coupons_show_type 为 1234 时显示 --
			 block v-else-if=storeCouponData.length  0 && showStoreCouponByType
				 !-- 红色波纹层：仅样式 234 显示；样式1 清新绿色用自身绿色环，不叠红色波纹避免出现红圈闪烁 --
				 view v-if=coupons_show_type !== '1' class=store-coupon-ripple-bg
					 view class=store-coupon-ripple-ringview
					 view class=store-coupon-ripple-ring store-coupon-ripple-ring-2view
					 view class=store-coupon-ripple-ring store-coupon-ripple-ring-3view
					 view class=store-coupon-ripple-dot v-for=i in 6 key='rd'+iview
				 view
				 !-- 样式1 清新绿色（仅绿色环，无红色波纹层） --
				 view v-if=coupons_show_type === '1' class=store-coupon-s1-wrap @click=goStoreCoupon
					 view class=store-coupon-s1-ringview
					 view class=store-coupon-s1-ring store-coupon-s1-ring-2view
					 view class=store-coupon-s1-ring store-coupon-s1-ring-3view
					 view class=store-coupon-s1-popup
						 view class=store-coupon-s1-header
							 ...
							 view class=store-coupon-s1-amount-wrap
								 text class=store-coupon-s1-currency¥texttext class=store-coupon-s1-amount{{ totalStoreCouponAmount }}texttext class=store-coupon-s1-unit元text
							 view
							 view class=store-coupon-s1-condition满{{ totalStoreCouponMin }}元可用view
						 view
						 view class=store-coupon-s1-white
							 view class=store-coupon-s1-timer
								 text class=store-coupon-s1-time{{ formatTime(storeCouponMinutes) }}texttext class=store-coupon-s1-septext
								 text class=store-coupon-s1-time{{ formatTime(storeCouponSeconds) }}texttext class=store-coupon-s1-septext
								 text class=store-coupon-s1-ms{{ formatTime(storeCouponMilliseconds, true) }}text
							 view
							 text class=store-coupon-s1-expire后失效text
							 view class=store-coupon-s1-btn立即领取view
						 view
					 view
				 view
				 !-- 样式2 限时抢购 --
				 view v-else-if=coupons_show_type === '2' class=store-coupon-s2-wrap @click=goStoreCoupon
					 view class=store-coupon-s2-ringview
					 view class=store-coupon-s2-ring store-coupon-s2-ring-2view
					 view class=store-coupon-s2-ring store-coupon-s2-ring-3view
					 view class=store-coupon-s2-popup
						 view class=store-coupon-s2-banner
							 ...
							 view class=store-coupon-s2-amount-wrap
								 text class=store-coupon-s2-currency¥texttext class=store-coupon-s2-amount{{ totalStoreCouponAmount }}texttext class=store-coupon-s2-unit元text
							 view
						 view
						 view class=store-coupon-s2-content
							 text class=store-coupon-s2-status你有待领取红包text
							 view class=store-coupon-s2-timer-box
								 text class=store-coupon-s2-timer-hint距离失效还剩text
								 view class=store-coupon-s2-timer
									 text class=store-coupon-s2-time{{ formatTime(storeCouponMinutes) }}texttext class=store-coupon-s2-septext
									 text class=store-coupon-s2-time{{ formatTime(storeCouponSeconds) }}texttext class=store-coupon-s2-septext
									 text class=store-coupon-s2-ms{{ formatTime(storeCouponMilliseconds, true) }}text
								 view
								 text class=store-coupon-s2-expire后失效text
							 view
							 view class=store-coupon-s2-btn立即使用view
						 view
					 view
				 view
				 !-- 样式3 节日气球 --
				 view v-else-if=coupons_show_type === '3' class=store-coupon-s3-wrap @click=goStoreCoupon
					 view class=store-coupon-s3-ringview
					 view class=store-coupon-s3-ring store-coupon-s3-ring-2view
					 view class=store-coupon-s3-ring store-coupon-s3-ring-3view
					 view class=store-coupon-s3-popup
						 view class=store-coupon-s3-balloons
							 view class=store-coupon-s3-balloonview
							 view class=store-coupon-s3-balloonview
							 view class=store-coupon-s3-balloonview
						 view
						 view class=store-coupon-s3-badge趣好卖view
						 view class=store-coupon-s3-header
							 ...
							 view class=store-coupon-s3-amount-wrap
								 text class=store-coupon-s3-currency¥texttext class=store-coupon-s3-amount{{ totalStoreCouponAmount }}texttext class=store-coupon-s3-unit元text
							 view
						 view
						 view class=store-coupon-s3-white
							 view class=store-coupon-s3-countdown-wrap
								 view class=store-coupon-s3-timer
									 text class=store-coupon-s3-time{{ formatTime(storeCouponMinutes) }}texttext class=store-coupon-s3-septext
									 text class=store-coupon-s3-time{{ formatTime(storeCouponSeconds) }}texttext class=store-coupon-s3-septext
									 text class=store-coupon-s3-ms{{ formatTime(storeCouponMilliseconds, true) }}text
								 view
								 text class=store-coupon-s3-expire后失效text
							 view
							 view class=store-coupon-s3-btn立即领取view
						 view
					 view
				 view
				 !-- 样式4 日常优惠（红包造型，对应 coupon-popup-1.html） --
				 view v-else class=store-coupon-s4-wrap @click=goStoreCoupon
					 view class=store-coupon-s4-ringview
					 view class=store-coupon-s4-ring store-coupon-s4-ring-2view
					 view class=store-coupon-s4-ring store-coupon-s4-ring-3view
					 view class=store-coupon-s4-particles
						 view class=store-coupon-s4-particle v-for=i in 6 key='p'+iview
					 view
					 view class=store-coupon-s4-popup
						 view class=store-coupon-s4-envelope
							 ...
							 view class=store-coupon-s4-amount-wrap
								 text class=store-coupon-s4-currency¥texttext class=store-coupon-s4-amount{{ totalStoreCouponAmount }}texttext class=store-coupon-s4-unit元text
							 view
							 text class=store-coupon-s4-condition满{{ totalStoreCouponMin }}元可用text
						 view
						 view class=store-coupon-s4-white
							 text class=store-coupon-s4-message领取后立即使用text
							 view class=store-coupon-s4-timer-wrap
								 view class=store-coupon-s4-timer
									 text class=store-coupon-s4-time{{ formatTime(storeCouponMinutes) }}texttext class=store-coupon-s4-septext
									 text class=store-coupon-s4-time{{ formatTime(storeCouponSeconds) }}texttext class=store-coupon-s4-septext
									 text class=store-coupon-s4-ms{{ formatTime(storeCouponMilliseconds, true) }}text
								 view
								 text class=store-coupon-s4-expire后失效text
							 view
							 view class=store-coupon-s4-btn立即领取view
						 view
					 view
				 view
				 view class=newuser-close-btn @click=handelClose('receive')
					 view class=newuser-close-btn-circleview
					 view class=newuser-close-btn-text{{ storeCouponCloseCountdown }}s 后关闭view
					 view class=newuser-close-icon×view
				 view
			 block
		 view
	  ...
	  uni-popup show=customerShow v-onhidePopup=handelClose('customer') v-if=customerShow
		  view class=version-popup
			  view class=close-on @click=handelClose('customer')
				  image
					  ...
					  image
			  view
			  image src=imgShare class=upgrade mode=widthFiximage
			  view class=save-picture @click=operator{{$t('lang.save_picture')}}view
		  view
	  uni-popup

	  uni-popup show=mpShareShow type=bottom v-onhidePopup=handelClose('mpshare')
		  view class=mp-share-warp
			  view class=lists
				  view class=list @click=onGoodsShare
					  image
						  src=httpsimages.eqingqu.comattachsphoto20240420240416_A4D37C1C7021179CF56E406972AFA295.png
						  mode=widthFiximage
					  text{{$t('lang.generate_sharing_poster')}}text
				  view
				  button class=list open-type=share
					  image
						  src=httpsimages.eqingqu.comattachsphoto20240420240416_DBA51C84B7EA458832C2966ADC6A714F.png
						  mode=widthFiximage
	  ...
	  view class= v-if=isShowEvaluate && !receiveStutas && !versionShow && !bonusShow && !customerShow && !mpShareShow
		  view class=cover @click=popCloseEvaluate catchtouchmove=trueview
		  view class=wxshare11
			  view class=textc style=position absolute;bottom 32rpx;left 10%;
				  view class=flex-center
					  view class=wxshare_2_11 mb24
						  view style=margin 48rpx 0 24rpx; class=co1b fs40 fs600 pl16求赞求鼓励！view
						  view class=co82 fs32 flex-center 您的鼓励是对我们最大的支持！view
					  view
				  view
				  view class=flex-center flexdc
					  view class=add_box3_a11  @click=goToAppStore评分view
					  view class=add_box3_1_a11 @click=popCloseEvaluate下次再说view
				  view
			  view
		  view
	  view
	  ...
	  block v-if=controlVersion
		  service @flaghanlde=prentflaghanlde v-if=shopConfig && shopConfig.consult.consult_set_state==1
			  shopConfig=shopConfig.consultservice
	  block

	  block v-if=click_add
		  view class=add_new style={'top'CustomBar + 10 +'px'}
			  view class=add_on{{$t('lang.remind')}}text @click=clickon{{$t('lang.remind1')}}textview
		  view
	  block
	  ...
	  view v-if=photo_show
		  view class=cover catchtouchmove=trueview
		  view class=wxshare_y style=background-color #ffffff;
			  view style=font-size 38rpx;margin 50rpx 50rpx 0rpx 50rpx;text-align center;温馨提示view
			  view style=font-size 28rpx;margin 50rpx;text-align center;是否打开拍照相册服务功能view
			  view style=display flex;flex-direction row;margin-bottom -20rpx;
				  view class=add_box3_buy_1_s @click=photo_showNo取消view
				  view class=add_box3_buy_1_y @click=photo_showYes确定view
			  view
		  view
	  view
	  !-- 小程序  H5 仍然在页面层渲染红包条 --
	  !-- #ifndef APP-PLUS --
	  sys-couponsys-coupon
	  !-- #endif --
	  custom-tabbar 
...
script
  ...
  import { incrementVersion } from '@utilsversion.js';
  import permision from @commonpermission.js
  import uniIcons from '@componentsuni-iconsuni-icons.vue'
  import uniPopup from '@componentsuni-popup.vue'
  import service from @componentscustomer-servicecustomer-service.vue;
  import uniPopups from '@componentsuni-popupuni-popup_1.vue';
  import dscCountDown from '@pagesDcomponentsvisualizationcount-downFrontend.vue'
   componentsvisualizationcount-downFrontend.vue
  import goodsServiceLabel from '@componentsgoods-service-labelgoods-service-label.vue'
  返回顶部
  import dscFilterTop from '@componentsdsc-filter-top'
  import sysCoupon from '@componentssys-coupon.vue'
  import {
	  pathToBase64,
	  base64ToPath
  } from '@commonimage-toolsindex.js'
  import {
	  compressImage
  } from '@commoncompressImage.js'
  import uniLoadMore from '@componentsuni-load-moreuni-load-more.vue';
  import universal from '@commonmixinsuniversal.js';
  import createPageTrackMixin from '@commonmixinspageTrackMixin.js';
  import dscRegion from '@componentsdsc-region.vue';
   #ifdef H5
  import isApp from '@configis-app.js'
   #endif
  import ImageCropper from '@pagesDcomponentsinvinbg-image-cropperinvinbg-image-cropper.vue'
  import CustomTabbar from '@componentscustom-tabbarcustom-tabbar.vue';
  import waterfall from '@pagesDcomponentswaterfall.vue'
  ...
  export default {
	  ...
	  data() {
		  return {
			  ...
			  showCover {},
				 GIF相关数据
				  showingGif null,
					 ...
					 scrollCheckThrottleMs 150,
			   scroll-view 下拉刷新 + 回顶（方案 A）
			  refresherTriggered false,
			  mainScrollTop 0,
			  refresherHintText '下拉刷新',

			   顶部区域背景：'gradient'=上红下白渐变；图片URL=PNGGIF，后续由后端返回
			  ...
			  vh 0,
			  keyword '',
			  indicatorStyle `height ${Math.round(uni.getSystemInfoSync().screenWidth(750100))}px;`,
			  ...
			  modulelist {
				  list [{
						  img 'httpsshhai2.oss-cn-shanghai.aliyuncs.comdatagallery_albumoriginal_img20230405_Lfu5kWvaxhNRPjh0eQqLmLe3Kv2Ii9.png'
					  },
					  {
						  img 'httpsshhai2.oss-cn-shanghai.aliyuncs.comdatagallery_albumoriginal_img20230405_Lfu5kWvaxhNRPjh0eQqLmLe3Kv2Ii9.png'
					  },
			  ...
			  newListTwo {},
			   videoList [{
			   	goods_thumb 'httpsshhai2.oss-cn-shanghai.aliyuncs.comdatagallery_albumoriginal_img20230405_Lfu5kWvaxhNRPjh0eQqLmLe3Kv2Ii9.png'
			   }],
			  videoListOne {
				  goods_thumb 'httpsshhai2.oss-cn-shanghai.aliyuncs.comdatagallery_albumoriginal_img20230405_Lfu5kWvaxhNRPjh0eQqLmLe3Kv2Ii9.png'
			  },
	  ...
	  created() {
		  ...
		  const isMobile = AndroidwebOSiPhoneiPadiPodBlackBerryIEMobileOpera Minii.test(navigator.userAgent);
	  ...
	  computed {
		   自定义下拉刷新区主文案（类 1688 品牌位）
		  refresherMarkText() {
			  const sc = this.shopConfig
			  if (sc && typeof sc === 'object') {
				  const name = sc.shop_title  sc.shop_name  sc.title
	  ...
	  onShareAppMessage(res) {
		  ...
		  let url = 'pagesmiddlesmiddles';
		  if (user_id) {
			  url = 'pagesmiddlesmiddlesparent_id=' + user_id
		  }

		  return {
			  title this.$store.state.common.shopConfig.shop_title,
			  path 'pagesmiddlesmiddles'
		  }
	  },
	  用户点击右上角分享朋友圈
	  ...
	  onLoad(e) {
		  ...
		  this.showCover = {};
		   ...
		   通知TabBar当前路由
		  uni.$emit('route-change', 'pagesmiddlesmiddles');
		  var that = this
		   确保在数据加载完成后初始化视频观察器
			 this.getCenterLine();
		  ...
		  uni.request({
			  url this.websiteUrl + 'apiapphome',
			  method 'GET',
			  data {
				  type 'home',
				  device 'app'
			  ...
			  success (res) = {
				  ...
				  for (var i = 0; i  that.modules.length; i++) {
				   ...
				   金刚区数据来源：apiapphome 返回的 modules 里 module==='nav' 的 data.list；若无 nav 或 list 为空则无数据
				  if (!list  !Array.isArray(list)) {
					  list = [];
					  if (!chunk) chunk = 10;
					  console.warn('金刚区无数据：接口未返回 nav 模块或 data.list 为空，请检查后台首页配置')
				  }
				   that.slideList = res.data.data[2].data.list
				   that.sellGoods = res.data.data[5].data
				   that.sellGoodImg = res.data.data[5].data.allValue.titleImg


				  var len = list.length;
				   var chunk = that.modules[3].data.isNumberSel  1;
				  var result = [];
				  var v = len  chunk;

				   之前轮播的时候自己做的分割
				   if (module3.data.isStyleSel == 2 && module3.data.isNumberSel) {
				   	for (var i = 0; i  len; i += chunk) {
				   		result.push(list.slice(i, i + chunk));
				   	}
				   }
				
				   this.mergedData = [...result[0], ...result[1]];
				  ...
				  that.aClass = arr
				   setTimeout(function() {
				   	for (let abc = 0; abc  that.visualdvAList.length; abc++) {
				   		uni.request({
				   			url that.websiteUrl + 'apivisualchecked',
				   			method 'POST',
				   			data {
				   				number that.visualdvAList[abc].data.allValue.selectGoodsId
				   					.length,
				   				goods_id that.visualdvAList[abc].data.allValue
				   					.selectGoodsId
				   					.toString()
				   			},
				   			header {
				   			...
				   			success (res1) = {
				   				...
				   				if (res1.data.data.length  0) {
				   					that.visualdvAList[abc].previewProlist = res1.data
				   						.data
				   				}
		  ...
		  setTimeout(() = {
			   ...
			   setTimeout(() = {
			   【修复】设置加载标志，防止 onShow 重复加载
			  that.isLoadingFirstPage = true;
			  that.getGoodsList(1);
			  that.getHotGoodsList(1);
		  ...
		  if (uni.getStorageSync('token') == ''  uni.getStorageSync('token') == undefined) {
			  if (isWeixin == true) {
				  ...
				  if (code == null  code == '') {
					   重新获取code
					  const base_url = encodeURIComponent('httpswww.ok9288.commobilepagesmiddlesmiddles');
					  const wx_url =
						   'httpsopen.weixin.qq.comconnectoauth2authorizeappid=wxd7daf1d30f51f1fe&redirect_uri=' + base_url + '&response_type=code&scope=snsapi_userinfo&state=123'+'&shopId=' + that.shopId + '#wechat_redirect'
						  'httpsopen.weixin.qq.comconnectoauth2authorizeappid=wx0b4564de207d9a5f&redirect_uri=' +
	  ...
	  onHide() {
		   首页切到其他 Tab 时，将所有 GIF 切换为封面图，减轻解码与绘制压力
  ...
  this.$nextTick(() = {
   ...
   重新计算高度
		  ...
		  if (this.page === 1 && !this.isLoadingFirstPage) {
			   如果 prolist 为空，说明是首次加载，onLoad 会处理，这里不重复调用
	  ...
	  onPullDownRefresh() {
		  this.performHomeRefresh()
	  },
	  onPageScroll(e) {
		  this.handleMiddlesScroll(e.scrollTop)
	  },
	  onReachBottom() {
		  this.tryLoadMoreGoods()
	  },
	  methods {
		  performHomeRefresh() {
			  var that = this
			  ...
			  setTimeout(() = {
				  ...
				  that.refresherHintText = '下拉刷新'
			  }, 100)
		  },
		  onRefresherRefresh() {
			  this.refresherTriggered = true
			  this.refresherHintText = '正在刷新…'
			  this.performHomeRefresh()
		  },
		  onRefresherPulling(e) {
			  if (this.refresherTriggered) return
			  const d = (e && e.detail)  {}
			  const dy = typeof d.dy === 'number'  d.dy  (typeof d.deltaY === 'number'  d.deltaY  0)
			  const thrPx = uni.upx2px  uni.upx2px(72)  48
			  this.refresherHintText = dy = thrPx  '松开刷新'  '下拉刷新'
		  },
		  onRefresherRestore() {
			  if (!this.refresherTriggered) {
				  this.refresherHintText = '下拉刷新'
			  }
		  },
		  scrollMainToTop() {
		  ...
		  onMainScroll(e) {
			  const st = e.detail && typeof e.detail.scrollTop === 'number'  e.detail.scrollTop  0
			  this.handleMiddlesScroll(st)
		  },
		  onMainScrollToLower() {
			  this.tryLoadMoreGoods()
		  },
		  ...
		  handleMiddlesScroll(scrollTop) {
			  const st = typeof scrollTop === 'number'  scrollTop  0
			  clearTimeout(this.scrollTimer)
			  this.isScrolling = true
			  const maxScroll = 200
			  const ratio = Math.min(1, st  maxScroll)
			  this.navOpacity = Number(ratio.toFixed(2))

			  var that = this
			  that.scrollState = st  800

			   #ifndef MP-WEIXIN
			  if (st = that.loginScrollThreshold &&
				  !uni.getStorageSync('token') &&
				  getApp().globalData.autoPromptLoginOnHome &&
				  that.homeFirstPageLoadedForLogin) {
				  getApp().globalData.autoPromptLoginOnHome = false
				  uni.navigateTo({ url 'pagesBloginlogindelta=1' })
			  }
			   #endif

			  if (!that.hasScrolled && st = that.scrollThreshold) {
		  ...
		  goRankingList() {
			  ...
			  uni.navigateTo({
				  url 'pagesCrankingListrankingListIndexhot_id=' + this.appHotId
			  })
		  },
		  ...
		  goSpeicalSell() {
			  ...
			  uni.navigateTo({
				  url 'pagesCspecialSalespecialSale'
		  ...
		  tapAllCategoriesItem(val) {
			  if (this.shopId  0) {
				  uni.navigateTo({
					  url 'pagesDshopshopGoodsshopGoodskeywords=' + val +
						  '&ru_id=' + this.shopId
				  })
			  } else {
				  uni.navigateTo({
					  url 'pagesDgoodslistgoodslistkeywords=' + val
				  })
			  }
		  },
		  ...
		  calculateEffectiveChars(str) {
			  const chinese = str.match([u4e00-u9fa5]g)  [];
		  ...
		  shopConfigHot() {
			  uni.request({
				  url this.websiteUrl + 'apishopconfig',
				  method 'GET',
				  data {
					   与购物车等页面保持一致，传递 app 版本号给后端
				  ...
				  success ({
					  ...
					  var that = this;
					   打印首页配置关键字段，方便排查 Android 端 tab-bar1 和 jiangbei 区域数据
					  ...
					  if (data.search_keywords) {
						  let newStr = 推荐,  + data.search_keywords;
						  this.search_keywords = newStr.split(',');
						  this.search_keywords_five = this.search_keywords.splice(0, 5);
					  }
					  if (data.hint_keywords) {
						   this.hint_keywords = data.hint_keywords.split(',')
						   this.words = data.hint_keywords.split(',')
		  ...
		  getoneList(num) {
			  ...
			  uni.request({
				  url that.websiteUrl + 'apiapphomezp_ys',
				  method 'GET',
				  data data,
				  header {
					  'Content-Type' 'applicationjson',
		  ...
		  handleToLogin(code) {
			  var that = this
			  uni.request({
				  url 'httpswww.ok9288.comapioauthh5_wechat_login',
				  method 'POST',
				  header {
		  ...
		  toSearch(e) {
			  ...
			  if (e == '热卖') {
				  uni.navigateTo({
					  url 'pagesDactivePageactivePageid=3'
				  })
			  } else if (e == '新品') {
				  uni.navigateTo({
					  url 'pagesDactivePageactivePageid=1'
				  })
			  }
		  },
		  ...
		  newGoodsListArr(res) {
			  for (let i = 0; i  res.length; i += 4) {
				  this.newArray.push(res.slice(i, i + 4));
			  }
		  ...
		  getNewGoodsList(page) {
			  ...
			  uni.request({
				  url this.websiteUrl + 'apigoodstype_list',
				  method 'GET',
				  data {
					  page reqPage,
		  ...
		  getHome_list() {
			  uni.request({
				  url this.websiteUrl + 'apiappad_positiontype=home_list',
				  method 'GET',
				  header {
					  'Content-Type' 'applicationjson',
					  'token' uni.getStorageSync('token'),
		  ...
		  getVideoList(page) {
			  ...
			  const reqSize = 10;
			  uni.request({
				  url this.websiteUrl + 'apigoodsgoodsvideo',
				  method 'POST',
				  data {
					  size reqSize,
					  page reqPage,
			  ...
			  uni.request({
				  ...
				  success (res) = {
					  let data = res.data.data
					   ...
					   this.hotListOne = data[0]
					   this.hotListTwo = data[1]
		  ...
		  getGoodsListWithCache(page) {
			  ...
			  if (page && page !== 1) {
			   ...
			   读取缓存
			  const cachedData = uni.getStorageSync('home_goods_list_cache');
			  const cacheTime = uni.getStorageSync('home_goods_list_cache_time');
			  const CACHE_EXPIRE_TIME = 5  60  1000;  缓存有效期5分钟
			  
			   检查缓存是否过期
			  const isCacheExpired = cacheTime && (Date.now() - cacheTime  CACHE_EXPIRE_TIME);
			  ...
			  uni.request({
				  ...
				  success (res) = {
						  ...
						  const hasChanged = that.compareGoodsList(cachedData, data);
							...
							if (!hasChanged) {
								...
								that.prolist = cachedData;
								 只在第一页初始化热卖严选商品，避免数据不足时变成 undefined
								if (that.page === 1) {
					  ...
					  if (that.page  1) {
					   ...
					   更新缓存（仅第一页）
					  if (that.page === 1) {
						  uni.setStorageSync('home_goods_list_cache', data);
						  uni.setStorageSync('home_goods_list_cache_time', Date.now());
						   【修复】第一页加载完成，清除加载标志
						  that.isLoadingFirstPage = false;
						  that.homeFirstPageLoadedForLogin = true;  标记首页数据已加载，用于滚动后跳转登录
						}
		  ...
		  getGoodsList(page) {
			  ...
			  success (res) = {
				  ...
				  data = data.map(item = {
				   ...
				   只在第一页初始化热卖严选商品，后续分页不覆盖，避免数据不足时变成 undefined
				  if (this.page === 1) {
					  this.hotListOne = data[8]  this.hotListOne
					  this.hotListTwo = data[9]  this.hotListTwo
				  }
		  ...
		  toShopHome function(item) {
			  ...
			  uni.navigateTo({
				  url 'pagesDshopshopHomeshopHomeru_id=' + item.user_id
			  })
		  },
		  linkvisualadv(item) {
			  let page, built
		  ...
		  linkHrefSearch(unm) {
			  ...
			  if (this.playingVideo) {
			   ...
			   uni.navigateTo({
			   	url 'pagesDactivePageactivePageid=23'
			   })
			   新品0 热销1  一件代发 3 严选4
			  if (unm == 0) {
				  uni.navigateTo({
					  url 'pagesDgoodslistgoodslistkeywords=新品'
				  })
			  } else if (unm == 1) {
				  uni.navigateTo({
					  url 'pagesDgoodslistgoodslistkeywords=热卖'
				  })
			  } else if (unm == 3) {
				  uni.navigateTo({
					  url 'pagesDgoodslistgoodslisttype=一件代发'
				  })
			  } else if (unm == 4) {
				  uni.navigateTo({
					  url 'pagesDgoodslistgoodslisttype=好卖严选'
				  })
			  }
		  },
		  linkHref(seckill_id, goods_id) {
			  如果是秒杀商品跳转到秒杀详情
			   if (seckill_id != 0) {
			   	uni.navigateTo({
			   		url 'pagesAseckilldetaildetailid=' + seckill_id +
			   			'&tomorrow=0&delta=2'
			   	});
			   } else {
			   	uni.navigateTo({
			   		url 'pagesCgoodsDetailgoodsDetailid=' + goods_id
			   	})
			   }
			  uni.navigateTo({
				  url 'pagesCgoodsDetailgoodsDetailid=' + goods_id
		  ...
		  linknav(item) {
			  ...
			  if (^https.test(page)) {
				  ...
				  this.$outerHref('pagesCarticledetaildetailid=' + this.articleId, 'app')
			  } else {
				  if (!item.media_live) {
					  this.$outerHref(page, built)
				  } else {
			  ...
			  if (platform == ios) { 如果机型是ios
				  ...
				  try {
					  console.log('[openTongZhi] 开始导入 iOS 类');
					   ...
					   参考 commonpermission.js 的实现，使用 app-settings URL scheme
					  ...
					  plus.ios.deleteObject(application);
						  ...
						  if (openResult === false  openResult === undefined  openResult === null) {
							  console.log('[openTongZhi] ⚠️ 方式2 返回 falseundefined，尝试方式3');
							  
							   方式3：尝试使用 App-Prefsroot=NOTIFICATIONS_ID（通知设置）
							   注意：App-Prefs 在 iOS 10+ 已被废弃，但某些版本可能仍可用
							  try {
								  console.log('[openTongZhi] 尝试方式3：使用 App-Prefsroot=NOTIFICATIONS_ID');
								  var settingsURL3 = NSURL.URLWithString(App-Prefsroot=NOTIFICATIONS_ID);
						  ...
						  uni.showModal({
										  title '打开设置',
										  content '无法自动打开设置，请手动前往：设置  通知  找到趣好卖  开启允许通知',
							  showCancel false,
										  confirmText '知道了'
									  });
								  }
		  ...
		  checkCommentApp() {
			  ...
			  uni.request({
				  url that.websiteUrl + 'apiappcommentis_comment_app',
				  method 'POST',
				  data {
					  device_id that.device_id,
					  model that.model,
		  ...
		  goToAppStore() {
			  var that = this
			   先调用提交接口
			  uni.request({
				  url that.websiteUrl + 'apiappcommentset_comment_app',
				  method 'POST',
				  data {
					  device_id that.device_id
				  },
		  ...
		  message() {
			  if (this.$isLogin()) {
				  uni.navigateTo({
					  url 'pagesBmessagelistmessagelist'
				  })
			  } else {
				  uni.showModal({
					  content 您需要登录会员!,
		  ...
		  searchFocus() {
			  ...
			  uni.navigateTo({
				  url 'pagesDsearchsearchshopId=' + this.shopId + '&currentWord=' + this.currentWord
			  });
		  },
		  async scanClick() {
			   #ifndef APP-PLUS
			  ...
			  uni.scanCode({
				  success (res) = {
					  if (this.$isLogin()) {
						  ...
						  this.keyword = res.result.replace(sg, );
						  if (res.scanType == 'EAN_13' && this.keyword.length == 13) {
							  if (this.shopId  0) {
								  uni.navigateTo({
									  url 'pagesDshopshopGoodsshopGoodskeywords=' + this
										  .keyword + '&ru_id=' + this.shopId
								  })
							  } else {
								  uni.navigateTo({
									  url 'pagesDgoodslistgoodslistkeywords=' + this
										  .keyword
								  })
							  }
							  return
					  ...
					  success (res) = {
						  if (this.$isLogin()) {
							  ...
							  if (res.scanType == 'EAN_13' && this.keyword.length == 13) {
								  if (this.shopId  0) {
									  uni.navigateTo({
										  url 'pagesDshopshopGoodsshopGoodskeywords=' +
											  this
											  .keyword + '&ru_id=' + this.shopId
									  })
								  } else {
		  ...
		  onSku(item) {
			  ...
			  if (that.attr.length  0) {
				  ...
				  uni.request({
					  url 'httpswww.ok9288.comapigoodsget_goods_barc_code',
					  method 'POST',
					  data {
					  ...
					  if (!roles.data.mobile_phone) {
						  uni.showModal({
							  ...
							  success res = {
								  if (res.confirm) {
									  uni.navigateTo({
										  url 'pagesBaccountsafebindphonebindphonedelta=1'
									  });
								  }
							  }
						  });
		  ...
		  goodsAloneSale() {
			  uni.navigateTo({
				  url `pagesDgoodslistgoodslistid=${this.goodsDetail.cat_id}`
			  })
		  },
		  closeAppDown() {
		  ...
		  getIsShow() {
			  uni.request({
				  url this.websiteUrl + 'apivisualappnav',
				  method 'POST',
				  header {
					  'Content-Type' 'applicationjson',
					  'token' uni.getStorageSync('token')  '',
		  ...
		  openApp() {
			  ...
			  window.location.href = 'dscmalltype=home';
			  微信内
...
.middles-page {
	 方案 A：与 scroll-view 主区域 flex 分配剩余高度（APP bouncenone 下页面不滚动） 
	height 100vh;
	display flex;
	flex-direction column;
	box-sizing border-box;
	 顶部区域：search + 金刚区 统一背景 
	 注意：不能用 overflowhidden，iOS 上会导致 fixed 的 search 被下方 waterFall 盖住 
	.header-with-bg {
		position relative;
	...
	.middles-main-scroll {
	 ...
	 自定义下拉刷新：去掉系统绿箭头，文案+轻量转圈（参考 1688 留白与层次） 
	.middles-refresher-slot {
		display flex;
	...
	.middles-refresher-inner {
		display flex;
		flex-direction column;
		align-items center;
		justify-content flex-end;
	}
	.middles-refresher-mark {
		font-size 30rpx;
		font-weight 600;
		color #c8c8c8;
		letter-spacing 4rpx;
	...
	.middles-refresher-hint {
		margin-top 10rpx;
		font-size 22rpx;
		color #999;
		line-height 1.2;
	}
	.middles-refresher-spinner {
		width 36rpx;
		height 36rpx;
		border 3rpx solid #e8e8e8;
		border-top-color #bbb;
		border-radius 50%;
		animation middles-refresher-spin 0.65s linear infinite;
	}
	@keyframes middles-refresher-spin {
		to {
			transform rotate(360deg);
		}
	}
	.middles-refresher-loading .middles-refresher-hint {
		margin-top 16rpx;
	}
	...
	.header-with-bg.has-header-bgnot(.header-scrolled) .input_view {
	...
	.header-with-bg.has-header-bg .king-kong-nav {
	 ...
	 金刚区移入 scroll-view 后：有顶部背景时保持白底卡片 
	.middles-page.middles-has-top-bg .middles-main-scroll .king-kong-nav {
		background-color #ffffff !important;
	}
	 有背景图：未滚动选中白色，滚动后选中红色 
	.header-with-bg.has-header-bgnot(.header-scrolled) .tab-bar1 .tab-item.active text {
		color #fff !important;
	}
	.header-with-bg.has-header-bgnot(.header-scrolled) .tab-bar1 .tab-item.active textafter {
		border-bottom-color #fff;
	}
	 仅在未滚动（彩色背景）时：柔和黑色系文字；滚动后白底用默认深色 
	.header-with-bg.has-header-bgnot(.header-scrolled) .placeholder_text {
		color #444 !important;
	}
	.header-with-bg.has-header-bgnot(.header-scrolled) .input_view image {
		filter brightness(0.4);
	}
	.header-with-bg.has-header-bgnot(.header-scrolled) .tab-bar1 .tab-itemnot(.active) text {
		color #444 !important;
	}
	.header-with-bg.has-header-bgnot(.header-scrolled) .tab-bar1 .all-text text,
	.header-with-bg.has-header-bgnot(.header-scrolled) .tab-bar1 .all-text .iconfont {
		color #444 !important;
	}
	.header-with-bg.has-header-bg.header-scrolled .tab-bar1 .tab-item.active text {
		color #f92028 !important;
	}
	.header-with-bg.has-header-bg.header-scrolled .tab-bar1 .tab-item.active textafter {
  ...
  .mp-share-warp button.listafter {
	  border none;
	  border-radius 0;
  }
  ...
  .adv-goods-list .scroll-view-item {
	  ...
	  overflow hidden;
	  border-radius 20upx;
	   box-shadow1upx 5upx 20upx 0 rgba(108,108,108,0.2); 
  }
  ...
  .visual-adv-swiper .adv-goods-list .scroll-view-itemlast-child {
	  margin-right 0;
  }
  ...
  .visual-adv-lie .adv-goods-list .scroll-view-itemnth-child(3n) {
	  margin-right 0;
  }

  .visual-adv-lie .adv-goods-list .scroll-view-itemnth-child(3n + 1) {
	   margin-left 3.5%;
  }

  .visual-adv-lie .adv-goods-list .swiper-slidelast-child,
  .visual-adv-lie .adv-goods-list .swiper-slidenth-last-child(2),
  .visual-adv-lie .adv-goods-list .swiper-slidenth-last-child(3) {
	  margin-bottom 0;
  }
  ...
  .product-new .tabs .itemlast-childafter {
	  height 0;
  }
  ...
  .goods-list .label-list .label-imglast-child {
	  margin-right 0;
  }
  ...
  .shipin_1 {
	  ...
	  border-radius 24rpx;
	   margin-right 16rpx;
	  background-image url(httpsimages.eqingqu.comattachsphoto20250320250331_C1DECA47A0B6E454FD79CA6F7D284A51.png);
  ...
  .notification-modal-btn-primaryactive {
	  opacity 0.8;
  }
  ...
  .ect-header-banner .btn-submit1hover {
	  border-color #fff;
  }
...
align-items center;
  ...
  .tab-bar1 .tab-item.active textafter {
	  content ' ';
	  position absolute;
...
.newuser-raynth-child(1) { top 15%; left 10%; animation-delay 0s; }
.newuser-raynth-child(2) { top 25%; right 12%; left auto; animation-delay 0.5s; }
.newuser-raynth-child(3) { top 45%; left 15%; animation-delay 1s; }
.newuser-raynth-child(4) { top 35%; right 18%; left auto; animation-delay 1.5s; }
.newuser-raynth-child(5) { top 65%; left 12%; animation-delay 2s; }
.newuser-raynth-child(6) { top 55%; right 15%; left auto; animation-delay 2.5s; }
...
.newuser-coinnth-child(1) { top 8%; left 8%; animation-delay 0s; }
.newuser-coinnth-child(2) { top 18%; right 12%; left auto; animation-delay 0.5s; }
.newuser-coinnth-child(3) { top 38%; left 12%; animation-delay 1s; }
.newuser-coinnth-child(4) { top 28%; right 8%; left auto; animation-delay 1.5s; }
.newuser-coinnth-child(5) { top 50%; left 5%; animation-delay 2s; }
.newuser-coinnth-child(6) { top 45%; right 5%; left auto; animation-delay 2.5s; }
...
.popup-mask {
 ...
 width 100%;
 height 100%;
 background-image url(httpsimages.eqingqu.comattachsphoto20250820250828_C11BFC195C7B2C001A52811B586FCBF4.png);
 background-size cover;
...
.receive-item {
 ...
 height 200rpx;
 max-width 200rpx;
 background-image url(httpsimages.eqingqu.comattachsphoto20250820250828_FD48E2E8CBD220D2E3BC7F68F9AC7CC6.png);
 background-repeat no-repeat;
...
.store-coupon-ripple-dotnth-child(4) { top 20%; left 10%; animation-delay 0s; }
.store-coupon-ripple-dotnth-child(5) { top 40%; right 15%; left auto; animation-delay 0.5s; }
.store-coupon-ripple-dotnth-child(6) { top 60%; left 20%; animation-delay 1s; }
.store-coupon-ripple-dotnth-child(7) { top 30%; right 25%; left auto; animation-delay 1.5s; }
.store-coupon-ripple-dotnth-child(8) { top 70%; left 15%; animation-delay 2s; }
.store-coupon-ripple-dotnth-child(9) { top 50%; right 10%; left auto; animation-delay 2.5s; }
@keyframes store-ripple-dot-float {
 0%, 100% { opacity 0; transform translateY(0) scale(0); }
 50% { opacity 1; transform translateY(-40rpx) scale(1); }
}

 ========== 普通优惠券样式1：清新绿色（宽度与 234 统一 680rpx） ========== 
...
.store-coupon-s2-btn {
 ...
 ========== 普通优惠券样式3：节日气球（宽度统一 680rpx，比例对照参考图） ========== 
...
@keyframes store-s3-ring-expand {
 ...
 ========== 样式3 节日气球：百分百对照 coupon-popup-2.html，宽度 640rpx 与 124 一致 ========== 
...
@keyframes store-s3-glow-pulse {
 ...
 气球：参考 top -80px right 20px，宽高 100px；气球 40x50，左 03060 top 0105 
...
.store-coupon-s3-balloonnth-child(1) {
 background linear-gradient(135deg, #FF6B9D, #FF3B5C);
 left 0;
 top 0;
 animation-delay 0s;
}
.store-coupon-s3-balloonnth-child(2) {
 background linear-gradient(135deg, #FFA07A, #FF6347);
 left 60rpx;
 top 20rpx;
 animation-delay 0.3s;
}
.store-coupon-s3-balloonnth-child(3) {
 background linear-gradient(135deg, #FFD700, #FFA500);
 left 120rpx;
 top 10rpx;
...
.store-coupon-s4-particlenth-child(1) { top 20%; left 10%; animation-delay 0s; }
.store-coupon-s4-particlenth-child(2) { top 40%; right 15%; left auto; animation-delay 0.5s; }
.store-coupon-s4-particlenth-child(3) { top 60%; left 20%; animation-delay 1s; }
.store-coupon-s4-particlenth-child(4) { top 30%; right 25%; left auto; animation-delay 1.5s; }
.store-coupon-s4-particlenth-child(5) { top 70%; left 15%; animation-delay 2s; }
.store-coupon-s4-particlenth-child(6) { top 50%; right 10%; left auto; animation-delay 2.5s; }
...
.store-coupon-s4-envelopebefore {
 content '';
 position absolute;
 top 0;
 left 0;
...
.store-coupon-s4-btn {
 ...
 样式4 日常优惠：APP 上宽度与 123 统一 680rpx 
 #ifdef APP-PLUS 
.store-coupon-s4-wrap {
 width 100%;
 max-width 680rpx;
...
.close .img {
 ...
 height 100rpx;
  ...
  .time {
	  ...
	  width 48rpx;
	   原 min-width24rpx 改为固定 width，值需根据实际字体大小调整 
	  background #ff4a4a;
	  color #fff;
	  text-align center;
	  padding 4rpx 0;
  ...
  .two {
	  height 132rpx;
	  display flex;
	  background-image url(httpsimages.eqingqu.comattachsphoto20250820250828_07F788811332A363E7FC4D69A4C6C4CC.png);
	  background-repeat no-repeat;
	  background-size 100% 100%;
	  margin-bottom 20rpx;
	  position relative;
  ...
  .two-text {
	  width 208rpx;
	  height 100%;
	  background-image url(httpsimages.eqingqu.comattachsphoto20250820250828_2FCB4A448F3907BB24B222052D73D31A.png);
	  background-repeat no-repeat;
	  background-size 100% 100%;
	  position absolute;
	  top 0;
...
.wxshare11 {
	...
	padding 63rpx 54rpx;
	box-sizing border-box;
	background url(httpsimages.eqingqu.comattachsphoto20260220260206_E5CE506D26CD5CB066A370A5AC6AF93F.png);
	background-size 100% 100%;
	background-repeat no-repeat;
