<template>
	<view class="content">
		...
		<view class="skip" @click="skipClick" v-if="num==list.length-1">
			<view>{{$t('lang.skip')}}</view>
			<uni-icons type="arrowright" size="18" color="#f92028"></uni-icons>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		mounted() {
			// this.adv();
			this.list =  [{
						ad_code: '/static/guide/1.jpg'
					},
					{
						ad_code: '/static/guide/2.jpg'
					},
					{
						ad_code: '/static/guide/3.jpg'
					},
					{
						ad_code: '/static/guide/4.jpg'
					},
					// {
					// 	ad_code: '/static/guide/5.jpg'
					// }
				]
		},
		methods: {
			...
			toTransition(e){
				if(this.num == this.list.length-1 && e.target.dx == 0){
					/**
					 * 向本地存储中设置launchFlag的值，即启动标识；
					 */
					uni.setStorage({
