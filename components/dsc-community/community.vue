				...
				<block v-if="list.length > 0">
					<view class="videolist-item flex_box fd_column jc_sb" v-for="(item,index) in list" :key="index" @click="goDetail(item.comment_id)">
						<view class="img">
							...
							<image src="https://shhai2.oss-cn-shanghai.aliyuncs.com/data/gallery_album/5/original_img/5_P_1705973846451.jpg" mode="aspectFill" class="image" v-else></image>
						</view>
...
export default {
	...
	methods: {
		...
		goDetail(id) {
			uni.navigateTo({
				url: `/pagesD/comment/commentList?id=${id}`
			})
		},
		loadMore() {
