...
export default {
  ...
  methods: {
    showCaptcha() {
      ...
      console.log('[captcha4] showCaptcha, captchaId:', mergedConfig.captchaId, 'platform:', platform);
        ...
        this.wv.overrideUrlLoading({ mode: "reject" }, function (e) {
          ...
          if (reqUrl.indexOf('gt4bridge://msg?') === 0) {
            try {
              ...
              console.log('[captcha4] Android URL scheme 消息:', data.type);
        ...
        this._plusMsgHandler = function (msg) {
          try {
            ...
            if (result.name == "postMessage") {
              if (result.arg.time === that._lastTime) {
                return;
              }
              that._lastTime = result.arg.time;
              console.log('[captcha4] iOS postMessage 消息:', result.arg.type);
              that._handleMessage(result.arg);
            }
          } catch (err) {
            console.error('[captcha4] iOS 消息处理异常:', err);
          }
        };
        plus.globalEvent.addEventListener("plusMessage", this._plusMsgHandler);
    ...
    _destroyWebview() {
      ...
      if (this._plusMsgHandler) {
        try {
          plus.globalEvent.removeEventListener("plusMessage", this._plusMsgHandler);
        } catch (e) {}
        this._plusMsgHandler = null;
      }
      if (this.wv) {
