<template>
	...
	<view :class="direction === 'center' ? 'centers' : ''" v-if="direction === 'center' ? open : true">
		<view class="product-window" :style="{ width: width + '%' }" :class="(open ? 'on' : '') + ' ' + direction">
			<!-- <view style="height: 100rpx;margin-top: -50rpx;margin-left: 27%;font-weight: bold;">选择支付方式</view> -->
			...
			<!-- <view class="iconfonts">
				<view style="display: flex;flex-direction: row;">
					<view style="color: #FF5050;margin-right: 6rpx;">{{ btnText }}</view>
					<view style="font-weight: bold;">s</view>
				</view>
			</view> -->
			<slot></slot>
		</view>
		<view class="mask" v-if="is_mask" @touchmove.prevent :hidden="!open"></view>
...
export default {
	...
	methods: {
		show() {
			var that = this;
			that.open = true;
			// that.btnText = that.btnText ? that.btnText : 10;
			// that.countdown = setInterval(() => {
			// 	if (that.btnText > 0) {
			// 		that.btnText = that.btnText;
			// 		--that.btnText;
			// 	} else {
