<template>
	<view class="easy-select" @click.stop="trigger" :style="[easySelectSize]">
		...
		<view class="easy-select-suffix" :style="{border: '1px solid rgba（0,0,0,0)'}" :class="[showSuffix]">
			<view class="easy-select-down-tag"><image src="https://images.eqingqu.com/attachs/photo/202404/20240416_89BF6904E05EEE5119E0BC3F987C8640.png" style="width: 22rpx;height: 22rpx;transform: rotate(90deg);"></image></view>
		</view>
		<view class="easy-select-options" v-if="showOptions" :style="{'min-width': boundingClientRect.width + 'px', top: optionsGroupTop, margin: optionsGroupMargin}">
			<view class="easy-select-options-item" v-for="item in options" :key="item.cat_id" @click.stop="select(item)" :class="{active: currentSelect.cat_id === item.cat_id}">
