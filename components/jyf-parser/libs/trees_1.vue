<template>
	<view :class="'interlayer '+(c||'')" :style="s">
		<block v-for="(n, i) in nodes" v-bind:key="i">
			...
			<video v-else-if="n.name=='video'" :id="n.attrs.id" class="video_1" :show-fullscreen-btn='false'
			...
			<audio v-else-if="n.name=='audio'" :ref="n.attrs.id" :class="n.attrs.class" :style="n.attrs.style" :author="n.attrs.author" :controls="n.attrs.controls" :loop="n.attrs.loop" :name="n.attrs.name" :poster="n.attrs.poster"
...
<script>
	...
	export default {
		...
		mounted() {
			var that = this
			for (that.top = that.$parent; that.top.$options.name != 'parser'; that.top = that.top.$parent);
		...
		methods: {
			init() {
				var that = this
				for (var i = that.nodes.length, n; n = that.nodes[--i];) {
					if (n.name == 'img') {
						that.top.imgList.setItem(n.attrs.i, n.attrs['original-src'] || n.attrs.src);
						// #ifdef APP-PLUS
						if (that.lazyLoad && !that.observer) {
							that.observer = uni.createIntersectionObserver(that).relativeToViewport({
								top: 500,
								bottom: 500
							});
							setTimeout(() => {
								that.observer.observe('._img', res => {
									if (res.intersectionRatio) {
										for (var j = that.nodes.length; j--;)
											if (that.nodes[j].name == 'img')
												that.$set(that.ctrl, j, 1);
						...
						} else if (that.$refs[n.attrs.id])
							ctx = that.$refs[n.attrs.id][0];
						if (ctx) {
							ctx.id = n.attrs.id;
							that.top.videoContexts.push(ctx);
						}
					}
				}
				// #ifdef APP-PLUS
