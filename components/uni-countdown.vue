<template name="uni-countdown">
	<view class="uni-countdown" :class="{'uni-countdown-not':notStyle}">
		...
		<view class="uni-countdown-splitor" :style="{color:dayTextColor || splitorColor}" v-if="mini && dType">天</view>
...
<script>
	export default {
		...
		methods: {
			load(){
				// 每次重新计算前先清掉旧的定时器，避免重复计时
				if (this.setTime) {
					clearInterval(this.setTime);
					this.setTime = null;
				}

				// timer 为空或非字符串时，不再继续执行，直接保持 00:00:00，防止后续 match 报错
				if (!this.timer || typeof this.timer !== 'string') {
					this.d = '00';
					this.h = '00';
					this.i = '00';
					this.s = '00';
			...
			countDown: function(self) {
				...
				if (leftTime > 0) {
					...
					hours = parseInt((leftTime / 1000) % 86400 / 60 / 60, 10);
					minutes = parseInt(leftTime / 1000 / 60 % 60, 10);
					seconds = parseInt(leftTime / 1000 % 60, 10);
				} else {
					// 活动已结束，统一归零，避免 day 为 undefined 导致界面出现 "undefined"
					day = 0;
					hours = 0;
					minutes = 0;
					seconds = 0;
