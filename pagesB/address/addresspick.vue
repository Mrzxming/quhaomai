				...
				<view class="address-content">
					<view class="left" @click="checkAddress(item.id,item)">
						<view class="title" style="display: flex;flex-direction: row;align-items: center;">
							...
							<view
								style="border-radius: 10rpx;border: 2rpx solid #F91F28;padding:0rpx 14rpx;text-align: center;color: #f91f28;transform: scale(0.8);margin-left: 20rpx;"
								v-if="iSaddress == item.id">默认</view>
						</view>
						<view style="height: 10rpx;"></view>
						<view class="address">{{ item.province_name }} {{ item.city_name }} {{item.district_name}}
							{{item.street_name}} {{item.address}}</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			checkAddress(val, item) {
				...
				uni.setStorageSync('newAddr', item)
				uni.navigateBack({
					delta:1
				})
				// uni.navigateTo({
...
<style>
	...
	.address-item .address-content .left {
		flex: 1;
	}

	.address-item .address-content .left view {
		line-height: 1.5;
		font-size: 26upx;
	}

	.address-item .address-content .title {
		font-weight: 700;
	}

	.address-item .address-content .title .name {
		margin-right: 15upx;
	}

	.address-item .address-content .address {
		color: #666666;
	}

	.address-item .address-content .right {
		display: flex;
		flex-direction: row;
	}

	.address-item .address-content .right view {
		line-height: 1.5;
		font-size: 26upx;
		padding: 20upx;
	}

	.address-info-show .uni-card {
		margin: 0;
	}

	.address-info-show .uni-card .uni-list-cell-navigate {
		padding: 0;
	}

	.address-info-show .uni-card .uni-list-cell-navigate .title {
		padding: 20upx 30upx;
		min-width: 100upx;
		font-size: 28upx;
	}

	.address-info-show .uni-card .uni-list-cell-navigate .value text {
		width: 100%;
	}

	.address-info-show .uni-card .uni-list-cell-navigate .value input {
		height: 40px;
		line-height: 40px;
	}

	.address-info-show .btn-bar {
		margin: 30upx 40upx;
		display: flex;
		flex-direction: column;
	}

	.address-info-show .btn-bar .btn {
		width: 100%;
		margin-bottom: 30upx;
	}
