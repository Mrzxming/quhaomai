<template>
	<view class="main">
		<block v-if="accountlog_list.length > 0">
			<view class="main_box">
				<view class="integral_t">{{$t('lang.current_shop')}}<text>{{type}}</text>{{$t('lang.current_shop1')}}
				</view>
				<view class="integral_t" v-if="expir">近30天即将过期{{expir}}积分
				</view>
