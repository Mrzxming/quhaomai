<template>
	<view class="user-recharge">
		<!-- <view class="header">
			<view :class="[currentIndex == index ? 'active_tab' : '']" v-for="(item, index) in tabData" :key="index"
				@click="changeChannel(index)">{{item}}</view>
		</view> -->

		<view class="pay_number bora14 ">
			<template>
				<span class="co62 pr10 fs28">提现至</span>
				<image class="pay_logo"
					src="https://images.eqingqu.com/attachs/photo/202404/20240416_DBA51C84B7EA458832C2966ADC6A714F.png">
				</image>
				<span class="fs32 pl10">微信</span>
				<input class="pay_field" type="text" v-model="payID" :placeholder="payID" disabled />
			</template>
		</view>

		<view class="get_money bora14">
			<view class="fs30 co27">{{$t('lang.deposit_money')}}</view>
			<view class="money_wrap">
				<view class="money_field" style=" padding: 20rpx 0; border-bottom: 1rpx solid #F1F1F1;">
					<text class="ico fs30 cored">¥</text>
					<input class="ipt" type="digit" v-model="amount" placeholder="请输入提现金额" placeholder-style="color:#626262;font-size:30rpx" />
		...
		<view class="unverified" v-if="msg && currentIndex == 2">
			...
			<navigator url="/pagesB/realname/realname" hover-class="none" class="go_auth">{{$t('lang.up_real_name')}}
			</navigator>
		</view>
		<!-- <button class="u-reset-button submit_btn" @click="thirdPartyLink" v-else>{{$t('lang.submit_apply')}}</button> -->
		...
		<uni-popups ref="popup" type="center">
			<view class="pop_content">
				...
				<view class="btn_wrap">
					...
					<button class="u-reset-button confirm_btn" :loading="showLoading"
						@click="beforeCloseHandle">{{loadingTxt}}</button>
				</view>
			</view>
		</uni-popups>
	</view>
	...
	export default {
		...
		computed: {
			...
			depositMoney: function() {
				if (this.buyer_cash > 0) return `¥${this.buyer_cash}`
		...
		methods: {
			...
			submitApply() {
				...
				// if (this.currentIndex == 0 && !this.payID) return uni.showToast({title: this.$t('lang.placeholder_wxpay'), icon: 'none'});
				...
				uni.request({
					...
					success: ({
						...
						if (data.code == 0) {
							...
							// }
							// this.account = data.user_money
							this.buyer_cash = data.buyer_cash
							this.payID = data.real_name
						} else {
							this.msg = data.msg
							uni.showToast({
								icon: 'none',
			...
			thirdPartyLink() {
				...
				uni.showToast({
					icon: 'none',
					title: '请下载【趣好卖APP】或打开微信小程序【庆趣批发】进行提现'
				})
				return;
				// #endif
				if (this.isCode !== 0) {
				...
				if (uni.getStorageSync('islogin') == 1) {
					...
					uni.getUserInfo({
						...
						success: (infoRes) => {
							uni.hideLoading();
							this.wechat_user_info = infoRes.userInfo
							if (Number(this.amount) >= Number(this.buyer_cash)) {
								if (this.currentIndex == 2) this.submitHandle()
								else this.$refs.popup.open()
							} else {
			...
			submitHandle(flag) {
				...
				// #ifdef MP-WEIXIN
				this.wechat_user_info.openId = uni.getStorageSync('scopeSession').openid
				this.wechat_user_info.unionId = uni.getStorageSync('scopeSession').unionid
				this.wechat_user_info.appid = "qingqupifa"
				// #endif
				...
				o.withdraw_type = 1
				o.withdraw_user_number = this.wechat_user_info.nickName
				o.real_name = this.payID
				// o.withdraw_user_number = this.payID
				o.wechat_user_info = JSON.stringify(this.wechat_user_info)
				// } else {
				// 	o.withdraw_type = 0
				// }
				console.log(o)
		...
		onShow() {
			this.$store.dispatch('setAccount').then(res => {
				this.account = res.data.user_money
			})
		},
		onLoad() {
			this.raplayInfo(1)
...
<style lang="scss" scoped>
	...
	.uni-list-cell-navigate .value textarea {
		width: 240px;
		height: 50px;
		margin-top: 10upx;
	}
