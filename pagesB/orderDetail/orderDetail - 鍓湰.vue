				...
				<view class="item-bd">
					...
					<view class="product-list product-list-max" v-if="listShow">
						<view class="product-items">
							<view class="item" hover-class="none" v-for="(goodsItem,goodsIndex) in orderDetail.goods"
								:key="goodsIndex" @click="productLink(goodsItem)">
									...
									<view class="p-virtual"
										...
										<view v-for="(virtualItem,virtualIndex) in goodsItem.virtual_goods"
											...
											<view class="virtual-item">{{$t('lang.virtual_card_number')}}：<text
													...
													class="copy" @click="copyCard(virtualItem.card_sn)">复制</text></view>
											<view class="virtual-item">
												{{$t('lang.virtual_card_pwd')}}：{{virtualItem.card_password}}<text
													class="copy" @click="copyCard(virtualItem.card_password)">复制</text>
											</view>
										</view>
									</view>
								</view>
			...
			<view class="uni-list">
				<view class="uni-list-cell">
					<view class="uni-list-cell-navigate">
						...
						<view class="value">
							<text class="mr10">{{ orderDetail.shipping_name }}</text><text
								class="uni-red">{{ orderDetail.shipping_fee_formated }}</text>
						</view>
					</view>
				</view>
		...
		<view class="btn-goods-action"
			...
			<view class="btn-bar">
				...
				<block v-if="orderDetail.handler == 2">
					<view class="btn btn-red" @click="receivedOrder(orderDetail.order_id)">{{$t('lang.received')}}
					</view>
				</block>
				<block v-if="orderDetail.handler == 4">
					<view class="btn btn-red">{{$t('lang.ss_received')}}</view>
				</block>

				<block v-if="orderDetail.handler == 1">
					<view class="btn btn-org" @click="onClickBigBtn(orderDetail.order_id)">{{$t('lang.cancel_order')}}
					</view>
				</block>
				<block v-if="orderDetail.handler == 3">
					<view class="btn btn-org" @click="onCommentBtn(orderDetail.order_id)">{{$t('lang.ping_ja')}}</view>
				</block>
				<block v-if="orderDetail.handler == 5">
					<view class="btn btn-org">{{$t('lang.payment')}}</view>
				</block>
				<block v-if="orderDetail.handler == 6">
					<view class="btn btn-org">{{$t('lang.confirmed')}}</view>
				</block>
				<block v-if="orderDetail.handler == 7">
					<view class="btn btn-org">{{$t('lang.canceled')}}</view>
				</block>
				<block v-if="orderDetail.handler == 8">
					<view class="btn btn-org">{{$t('lang.invalid_order')}}</view>
				</block>
				<block v-if="orderDetail.tracker">
					<view class="btn btn-org" @click="orderTracking">{{$t('lang.order_tracking')}}</view>
				</block>
			</view>
		</view>
		<!-- 门店二维码弹框 -->
