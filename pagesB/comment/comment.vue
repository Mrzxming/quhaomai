<template>
	<view class="container comment_content">
		<view class="flex-between plr32" style="height: 80rpx;background: #FFFAD5;" v-if="commentList.length > 0">
			<view class="flex-between cored fs28">
				<image src="https://images.eqingqu.com/attachs/photo/202511/20251126_6A0A3112607D38738E5E8419A2F2926B.png" mode="heightFix" class="img36 "></image>
				<span class="ml20">快来评价吧! 还有积分获得哦！</span>
			</view>
		</view>
		<view class="product_list">

			<view class="tabs">
				<view :class="['tab_item', activeTab == 0 ? 'active_tab' : '']" @click="switchTab(0)">待评价<text
						class="await_count">{{ signNum && activeTab == 0 ? ' · ' + signNum : '' }}</text></view>
				<view :class="['tab_item', activeTab == 1 ? 'active_tab' : '']" @click="switchTab(1)">
					已评价{{showEvaluate ? '/追评' : ''}}</view>
			</view>

			<template v-if="activeTab == 0">
				<view :class="['product_item', index > 0 ? 'u-border-top' : '']" v-for="(item,index) in commentList"
					:key="index" @click="goLink(`/pagesC/goodsDetail/goodsDetail?id=${item.goods_id}`)">
						...
						<view class="goods_name">
							...
							<view class="btns" v-if="item.can_evaluate">
								<view class="evaluate_btn"
									@click.stop="goLink(`../commentDetail/commentDetail?id=${item.rec_id}&type=0`)">评价
								</view>
							</view>
						</view>
					</view>
			...
			<template v-if="activeTab == 1">
				<view :class="['product_item', 'have_evaluation', index == 0 ? 'border_top_0' : '']"
					v-for="(item,index) in commentList" :key="index"
					@click="goLink(`/pagesC/goodsDetail/goodsDetail?id=${item.goods_id}`)">
					<view class="g_title">{{item.shop_name}}</view>
					<view class="goods_info">
						<image class="goods_img" :src="item.goods_thumb"></image>
						<view class="goods_name">
							...
							<view class="rate_wrap">
								<text>评分</text>
								<text
									:class="['iconfont', 'icon-collection-alt', 'size_24', rIndex < item.comment_rank ? 'color_red' : '']"
					...
					<block v-for="(comItem,comIndex) in item.comment" :key="comIndex">
						<view class="pro_content" v-if="comItem.add_comment_id == 0">
							...
							<view class="img_list" v-if="comItem.comment_img_list.length">
								<image class="goods_img" v-for="(imgItem,imgIndex) in comItem.comment_img_list"
									:key="imgIndex" :src="imgItem.comment_img"></image>
							</view>
							<view class="btn_wrap" v-if="item.can_add_evaluate">
								<view class="additional_review"
									@click.stop="goLink(`../commentDetail/commentDetail?id=${item.rec_id}&type=1`)">追评
								</view>
							</view>
						</view>
...
<script>
	...
	export default {
		...
		onShow() {
			// 修复：监听评价成功事件，刷新数据并切换到已评价tab
			uni.$on('commentSuccess', (data) => {
				if (data && data.have) {
					this.activeTab = 1; // 切换到已评价tab
					this.page = 1; // 重置页码
					this.commentListHandle(); // 刷新列表
				}
				// 移除事件监听，避免重复触发
				uni.$off('commentSuccess');
			});
		},
		onUnload() {
			// 页面卸载时移除事件监听
