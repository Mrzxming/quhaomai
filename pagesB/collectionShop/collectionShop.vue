	...
	<view>
		...
		<view class="shop">
			<view class="shop-top" v-for="(item,index) in shopCollectList" :key="index">
				<view class="left" @click="$outerHref('/pagesD/shop/shopHome/shopHome?ru_id='+item.ru_id,'app')">
					<image :src="item.shop_logo"></image>
					<view class="info">
						<view class="name uin-ellipsis">{{ item.shoprz_brand_name }}{{ item.shop_name_suffix }}<text style="background-color: #FE0302;color: #ffffff;border-radius: 10rpx;padding: 0rpx 12rpx;font-size: 22rpx;margin-right: 10rpx;margin-left: 10rpx;" v-if="item.shop_grade==13&&item.shop_name_len < 8">{{item.shop_grade_name}}</text></view>
						<view v-if="item.shop_name_len > 8"><text style="background-color: #FE0302;color: #ffffff;border-radius: 10rpx;padding: 0rpx 12rpx;font-size: 22rpx;margin-right: 10rpx;margin-left: 10rpx;width: 110rpx;" v-if="item.shop_grade==13">{{item.shop_grade_name}}</text></view>
