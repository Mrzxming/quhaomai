<template>
	<view class="container comment_detail_content">
		...
		<view class="floor_bar">
			<view :class="['btn', isSubmitting ? 'btn_disabled' : '']" @click="btnSubmit" :style="{opacity: isSubmitting ? 0.6 : 1}">
				<text v-if="isSubmitting">提交中...</text>
				<text v-else>{{$t('lang.comment_submit')}}</text>
			</view>
		</view>
	</view>
</template>
	...
	export default {
		data() {
			return {
				...
				isAddEvaluate: 0,
				tagList: [],
				isSubmitting: false, // 提交状态，防止重复点击
				satisfactionList: [
					{
						title: '商品描述相符度',
						rank: 0
				...
				if(this.rank == 0){
					...
					this.$store.dispatch('setAddgoodscomment',{
						...
						if(res.data.error == 0){
							...
							setTimeout(() => {
								// 修复：使用 navigateBack 返回上一页，保留页面栈，确保返回按钮正常显示
								// 通过事件通知 comment 页面刷新数据并切换到已评价tab
								uni.$emit('commentSuccess', { have: 1 });
								uni.navigateBack({
									delta: 1
								});
							}, 2000)
