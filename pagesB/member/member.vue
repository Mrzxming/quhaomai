...
<script>
	...
	export default {
		...
		mounted() {
			...
			setTimeout(() => {
				if (this.drpChangeCard && this.drpChangeCard.list && this.drpChangeCard.list.length > 0) {
					console.log('mounted 中处理数据');
					this.processMembershipData();
				} else {
					console.log('mounted 中数据未就绪');
				}
			}, 100);
		},
		computed: {
		...
		watch: {
			...
			drpChangeCard: {
				handler(newVal) {
					console.log('drpChangeCard 变化:', newVal);
		...
		async onLoad() {
			try {
				...
				// 同时加载两种数据源
				await Promise.all([
					this.$store.dispatch('setDrp').catch(err => {
						console.warn('Vuex 数据加载失败:', err);
					}),
				...
				console.log('drpdata:', this.drpdata);
				console.log('setdrp:', this.setdrp);
				console.log('mergedDrpData:', this.mergedDrpData);
		...
		async onShow() {
			...
			// 【新增】缓存逻辑：检查是否有缓存数据（不依赖isShowList）
			const token = uni.getStorageSync('token');
			// 检查内存缓存和本地存储缓存（无论是否有token都检查）
			const hasMemoryCache = this.cachedCoulist.length > 0 || this.cachedGoodslist.length > 0;
			const hasStorageCache = uni.getStorageSync('member_data_cache');
			...
			console.log('显示前的token', uni.getStorageSync('token'))
			if (uni.getStorageSync('token') != '' && uni.getStorageSync('token') != undefined) {
				this.isShowList = true;
				console.log('显示后的token', uni.getStorageSync('token'))
				console.log('显示后的 this.isShowList', this.isShowList)
				try {
					// 并行执行基础数据请求
					await Promise.all([
						this.getList(),
		...
		updateMemberStatus() {
			...
			if (isMemberStatus) {
				...
				if (cardInfo && cardInfo.id) {
					...
					console.log('会员状态已更新，会员卡ID:', this.currentMemberCardId, '轮播索引:', this.currentSwiperIndex);
				}
				this.isMember = true;
			} else {
				// 非会员
				...
				console.log('非会员状态，轮播索引重置为2（年度会员）');
			}
		},
					...
					uni.request({
						...
						success: (res) => {
							if (res.data.status == 'success') {
								console.log('drpcard 数据获取成功:', res.data.data);
			...
			async ensureDataAndShowPopup() {
				try {
					...
					// 显示弹窗
					if (this.mergedDrpData && this.mergedDrpData.expiry && this.mergedDrpData.expiry.expiry_status && this.mergedDrpData
			...
			processMembershipData() {
				try {
					...
					console.log('会员数据处理完成，默认选中:', this.selectedMembership);
			...
			handleSubscription() {
				...
				// 处理订阅逻辑
				console.log('选择的会员套餐:', this.selectedMembership)
				uni.showModal({
					title: '确认开通',
					...
					uni.request({
						...
						success: (res) => {
							if (res.data.status == 'success') {
								...
								console.log('getdrp 设置的数据:', this.setdrp);
								resolve(res.data.data);
							} else {
			...
			onSwiperChange(e) {
				...
				if (!this.isMember) {
					...
					this.$nextTick(() => {
						...
						console.log('非会员切换轮播到索引:', this.currentSwiperIndex);
						// 延迟重置标志位，确保事件处理完成
						setTimeout(() => {
					...
					this.$nextTick(() => {
						...
						if (this.currentSwiperIndex !== targetIndex) {
							...
							console.log('会员状态，强制回到轮播索引:', targetIndex);
						}
						// 延迟重置标志位，确保事件处理完成
						setTimeout(() => {
							this.isSwiperChanging = false;
			...
			handleOneClickReceive() {
				console.log('当前选中的轮播项索引:', this.currentSwiperIndex);
			...
			renew_method_select(type) {
				...
				setTimeout(() => {
					...
					if (this.renew_type == 'buy') {
						uni.navigateTo({
							url: `/pagesA/drp/done/done?receive_type=buy&apply_status=renew&membership_card_id=${this.selectedMembership.id}`
						})
						this.renewShow = false
					}
					// 积分兑换
					if (this.renew_type == 'integral') {
						...
						o = {
							receive_type: this.type,
							membership_card_id: this.selectedMembership.id,
							pay_point: this.selectedMembership.integralValue // 使用积分值而不是金额
						}
						this.$store.dispatch('setDrpApply', o).then(data => {
							let res = data.data
							if (res.status == 'success') {
			...
			async loadMemberInfo() {
				try {
					await this.getdrp();
					// getdrp成功后会自动触发setdrp的watch，进而调用updateMemberStatus
					console.log('通过getdrp加载会员信息成功');
				} catch (error) {
			...
			restoreCacheFromStorage() {
				try {
					const cached = uni.getStorageSync('member_data_cache');
			...
			async silentUpdateMemberData() {
				try {
					...
					console.log('后台静默更新会员数据', res.data.data);

					// 检查各数据块是否有变化
					let hasCoulistChanged = this.compareAndUpdateCoupons(
						res.data.data.coulist_month.list, this.cachedCoulist
					);
					let hasSecondlistChanged = this.compareAndUpdateCoupons(
						res.data.data.coulist_quarter.list, this.cachedSecondlist
					);
					let hasThridlistChanged = this.compareAndUpdateCoupons(
						res.data.data.coulist_year.list, this.cachedThridlist
					);
					let hasGoodslistChanged = this.compareAndUpdateGoods(
						res.data.data.goodslist, this.cachedGoodslist
					);

					// 只在数据真正变化时才更新UI（使用$set确保响应式，全平台兼容）
					if (hasCoulistChanged) {
						console.log('月度优惠券数据有变化，后台静默更新UI');
						// 使用cou_id匹配，支持顺序变化
						const newCoulist = this.updateCouponListData(this.coulist, res.data.data.coulist_month.list, 'cou_id');
						this.$set(this, 'coulist', newCoulist);
						this.cachedCoulist = [...res.data.data.coulist_month.list];
					}

					if (hasSecondlistChanged) {
						console.log('季度优惠券数据有变化，后台静默更新UI');
						const newSecondlist = this.updateCouponListData(this.secondlist, res.data.data.coulist_quarter.list, 'cou_id');
						this.$set(this, 'secondlist', newSecondlist);
						this.cachedSecondlist = [...res.data.data.coulist_quarter.list];
					}

					if (hasThridlistChanged) {
						console.log('年度优惠券数据有变化，后台静默更新UI');
						const newThridlist = this.updateCouponListData(this.thridlist, res.data.data.coulist_year.list, 'cou_id');
						this.$set(this, 'thridlist', newThridlist);
						this.cachedThridlist = [...res.data.data.coulist_year.list];
					}

					if (hasGoodslistChanged) {
						console.log('商品数据有变化，后台静默更新UI');
						// 商品列表：使用goods_id匹配，支持商品顺序变化
						...
						// 遍历新数据，更新或添加
						const newGoodslist = [];
						res.data.data.goodslist.forEach((newItem) => {
			...
			async loadUserProfile() {
				...
				// 【调试提示】如果会员状态不正确，请在此处检查 data.drp_shop 的值
				// console.log('loadUserProfile - drp_shop:', data?.drp_shop)
