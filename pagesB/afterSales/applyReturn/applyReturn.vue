<template>
	<view v-if="loading">
		<view class="user-detail">
				...
				<view class="product-items">
					<view class="item" v-for="(goodsInfo,index) in goodsList" :key="index">
						...
						<view class="product-info">
							...
							<view class="price" v-if="goodsInfo.goods_coupons > 0">-
								{{ goodsInfo.formated_goods_coupons }}
							</view>
							<view class="price" v-if="goodsInfo.goods_bonus > 0">- {{ goodsInfo.formated_goods_bonus }}
							</view>
							<view class="price" v-if="goodsInfo.goods_favourable > 0">-
								{{ goodsInfo.formated_goods_favourable }}
							</view>
							<view class="price" v-if="goodsInfo.value_card_discountformated_goods_coupons > 0">-
								{{ goodsInfo.formated_value_card_discount }}
							</view>
							<view class="price"
								v-if="goodsInfo.goods_coupons > 0 || goodsInfo.goods_bonus > 0 || goodsInfo.goods_favourable > 0 || goodsInfo.value_card_discount > 0">
								{{ $t('lang.return_total') }}：{{ goodsInfo.formated_should_return }}
							</view>
				...
				<view class="padding-all">
					...
					<view class="select-one-1">
						<view class="ect-selects">
							<view class="ect-select" :class="{'active':item.cause == retrun_cause_id}"
								v-for="(item,index) in goods_cause" @click="causeSelect(item.cause)" :key="index">
								<text>{{ item.lang }}</text>
							</view>
						</view>
					</view>
			...
			<view class="uni-card uni-card-not" v-if="retrun_cause_id==3">
				<view class="padding-all">
					<view class="h4">自定义退款货品金额<text class="dot color-red">*</text></view>
					<view class="select-one-1">
						<view style="display: flex;flex-direction: row;justify-content: space-between;">
							<view class="ect-selects">
								<input :disabled="isdisabled" placeholder-style="color: #d1d1d1;font-size: 30rpx;"
									class="tui-input" :placeholder="'不超过'+ allMoney+'元'" @blur="toMoney" :value="money"
									type="number" maxlength="10" />
							</view>
							<view style="display: flex;flex-direction: row;text-align: right;">
								<view style="width: 100rpx;" @click="toAll">全部</view>
								<view style="width: 100rpx;" @click="toAllNo">重输</view>
							</view>
						</view>
					</view>
				</view>
			...
			<view class="uni-card uni-card-not" v-if="retrun_cause_id==3">
				<view class="padding-all">
					<view class="h4">自定义退款运费金额<text class="dot color-red">*</text></view>
					<view class="select-one-1">
						<view style="display: flex;flex-direction: row;justify-content: space-between;">
							<view class="ect-selects">
								<input :disabled="isdisabled2" placeholder-style="color: #d1d1d1;font-size: 30rpx;"
									class="tui-input" :placeholder="'不超过'+ allMoney2+'元'" @blur="toresMoney"
									:value="resmoney" type="number" maxlength="10" />
							</view>
							<view style="display: flex;flex-direction: row;text-align: right;">
								<view style="width: 100rpx;" @click="toAll2">全部</view>
								<view style="width: 100rpx;" @click="toAllNo2">重输</view>
							</view>
						</view>
					</view>
				</view>
			...
			<view class="uni-card uni-card-not">
				<view class="padding-all">
					...
					<view class="select-one-1">
						<view class="select-one-1">
							<view class="ect-selects">
								<view class="ect-select" :class="{'active':item.cause_id == causeSelected}"
									v-for="(item,index) in parent_cause" @click="causeSelect2(item.cause_id)"
									:key="index"><text>{{ item.cause_name }}</text></view>
							</view>
						</view>
					</view>
				</view>
				...
				<view class="padding-all">
					...
					<view class="select-one-1">
						<view class="uni-number-min">
							<uni-number-box :value="value" :min="1" :max="applyRefoundDetail.return_goods_num" :step="1"
								:disabled="applyRefoundDetail.return_goods_num < 2"
			...
			<view class="uni-card uni-card-not">
				<view class="padding-all">
					...
					<view class="value"><textarea :placeholder="$t('lang.problem_desc')"
							v-model="return_brief"></textarea></view>
				</view>
			</view>
			<view class="uni-card uni-card-not">
				<view class="padding-all">
			...
			<view class="uni-card uni-card-not">
				<view class="padding-all">
					...
					<p class="f-03 col-7 m-top06">
						{{$t('lang.pic_prompt_notic_one')}}<br>{{ $t('lang.pic_prompt_notic_two') }}{{ returnPictures }}{{ $t('lang.pic_prompt_notic_two2') }}
					</p>
				</view>
			</view>
			<block v-if="consignee">
				<view class="uni-card uni-card-not"
					v-if="retrun_cause_id != '' && (retrun_cause_id == 0 || retrun_cause_id == 2)">
					<view class="padding-all">
						...
						<view class="uni-list address-info-show">
							<view class="uni-list-cell uni-list-cell-title uni-list-cell-right">
								<view class="uni-list-cell-navigate">
									...
									<view class="value"><input :placeholder="$t('lang.enter_consignee')"
							...
							<view class="uni-list-cell uni-list-cell-title uni-list-cell-right">
								<view class="uni-list-cell-navigate">
									...
									<view class="value"><input :placeholder="$t('lang.enter_mobile')" v-model="mobile">
									</view>
								</view>
							</view>
							<view class="uni-list-cell uni-list-cell-title uni-list-cell-right"
								@click="handleRegionShow">
								<view class="uni-list-cell-navigate uni-navigate-right">
									<text class="title">{{ $t('lang.region_alt') }}</text>
									<view class="value" :class="{'gay': !regionSplic}">
										{{ regionSplic ? regionSplic : $t('lang.select') }}
									</view>
								</view>
							</view>
							<view class="uni-list-cell uni-list-cell-title uni-list-cell-right">
								<view class="uni-list-cell-navigate">
									...
									<view class="value"><input :placeholder="$t('lang.enter_address')"
											v-model="address"></view>
								</view>
							</view>
						</view>
					</view>
				...
				<mpvue-picker :themeColor="themeColor" ref="mpvuePicker" :mode="mode" :deepLength="deepLength"
					:pickerValueDefault="pickerValueDefault" @onConfirm="onConfirm($event,curRegion)"
					@onCancel="onCancel" :pickerValueArray="pickerValueArray"></mpvue-picker>
			</block>
			<view class="uni-card uni-card-not">
				<van-field v-model="return_remark" label="留言" :placeholder="$t('lang.enter_message')" type="textarea"
					class="my-bottom" />
			</view>
			<view class="padding-all">
			...
			<view class="btn-bar btn-bar-fixed" style="background-color: #FFFFFF;">
				<!-- <view class="btn btn-red" @click="handleClick" :disabled="isdisabled">{{$t('lang.submit_apply')}}</view> -->
				<button class="btn btn-red" @click="handleClick"
					:disabled="isdisabledbtn">{{$t('lang.submit_apply')}}</button>
			</view>
		</view>
		<dsc-common-nav></dsc-common-nav>

		<!--地区选择-->
		<dsc-region :display="regionShow" :regionOptionData="regionData" @updateDisplay="getRegionShow"
			@updateRegionDate="customUpdateRegion" v-if="regionLoading"></dsc-region>
	</view>
</template>
...
<script>
	...
	export default {
		...
		computed: {
			...
			returnGoodsNum() {
				return this.applyRefoundDetail.order && this.applyRefoundDetail.order.shipping_status == 0 ? this
					.applyRefoundDetail
		...
		methods: {
			...
			handleClick() {
				...
				if (!this.debounceFn) {
					this.debounceFn = this.debounce(this.submitBtn, 500);
				}
				this.debounceFn();
			},
			toAll: function() {
			...
			toresMoney: function(e) {
				// if (this.causeSelected == 8) {
				console.log(parseFloat(e.detail.value))
				if (parseFloat(e.detail.value) > parseFloat(this.applyRefoundDetail.order.shipping_fee)) {
					uni.showToast({
						title: '当前金额达到退款最大金额',
						icon: "none"
					})
					// this.isdisabled2 = true
					this.resmoney = this.applyRefoundDetail.order.shipping_fee
					return;
				} else {
					this.resmoney = e.detail.value
				}
				// }
				console.log('resmonr', this.resmoney);
			},

			toMoney: function(e) {
				if (parseFloat(e.detail.value) > parseFloat(this.allMoney)) {
					uni.showToast({
						title: '当前金额达到退款最大金额',
						icon: "none"
					})
				...
				// 		})
				// 		this.isdisabled = true
				// 		this.money = this.applyRefoundDetail.order.shipping_fee
				// 		console.log(888,this.applyRefoundDetail.order.shipping_fee);
				// 		console.log(999,this.money);
				// 		return;
				// 	}else{
				// 		this.money = e.detail.value
				// 	}
			...
			causeSelect(id) {
				...
				if (this.retrun_cause_id == 3) {
					var abc = 0;
					// var efg = 0
					for (var a = 0; a < this.goodsList.length; a++) {
						var d = 0
						d = this.goodsList[a].goods_price * this.goodsList[a].goods_number
						abc = d + abc
						// var e = 0
						// e = this.goodsList[a].goods_surplus - d
						// efg =  e + abc
					}
					this.allMoney = abc.toFixed(2)
					// this.allMoney2 = efg.toFixed(2)
					this.allMoney2 = this.applyRefoundDetail.order.shipping_fee
				}
			},
			causeSelect2(id) {
				console.log('退换货原因', id)
				this.causeSelected = id
				if (this.causeSelected == 8) {
					// if(parseFloat(money_1)>=parseFloat(this.applyRefoundDetail.order.shipping_fee)){
					// 	uni.showToast({
					// 		title: '当前金额达到退款最大金额',
					// 		icon: "none"
					// 	})
			...
			submitBtn() {
				let o;
				if (this.retrun_cause_id == 3) {
					if (this.money == '' && this.causeSelected !== 8) {
						uni.showToast({
							title: '请填写退款货品金额',
							icon: 'none'
						})
						return;
					} else if (this.resmoney == '') {
						uni.showToast({
							title: '请填写退款运费金额',
							icon: 'none'
						})
						...
						o = {
							return_shipping_fee: this.resmoney,
							return_price: this.money,
							rec_id: this.rec_id,
				...
				uni.request({
					...
					success: (res) => {
						...
						if (res.data.status == 'success') {
							...
							// 如果返回失败，也显示错误信息
							uni.showToast({
								title: res.data.errors?.message || '提交失败',
								icon: "none"
							})
						}
						// 无论成功失败，都等待2秒后返回上一页
				...
				uni.chooseImage({
					...
					success: (res) => {
						...
						if (that.materialList.length + 1 > this.returnPictures) {
							uni.showModal({
								title: this.$t('lang.hint'),
								content: this.$t('lang.return_max_pic_prompt') + this.returnPictures +
									this.$t('lang.return_max_pic_prompt2')
							});
							return false;
						}
