	...
	<view class="container-tab-bar">
		...
		<block v-if="couponData && couponData.length>0">
			<view class="coupons_list">
				<view class="coupons_item" v-for="(item,index) in couponData" :key="index" :class="{'g-gay':status != 0}">
					<view class="coupons_left">
						<view class="coupons_left_top">
							<view class="coupon_price" v-if="item.cou_type==5">包邮券</view>
							<view v-else>
								<currency-price :price="getCouponDisplayMoney(item)" :size="24"></currency-price>
							</view>
							<view class="collect_coupon" v-if="item.cou_type_name && item.cou_type_name !== ''">
						...
						<view class="coupons_goods_name">
							...
							<view class="coupon_tag2" v-if="item.store_name && item.store_name !== ''" style="font-size: 24rpx; margin-top: 8rpx;">
								{{item.store_name}}
							</view>
						</view>
						<view class="coupons_limited_period">
							<text>{{item.begintime}}{{$t('lang.zhi')}}{{item.endtime}}</text>
						</view>
					</view>
					<view class="coupons_right">
						<view class="get_coupon_time">
							<button class="u-reset-button coupon_btn" v-if="status == 0" @click="couponLink(item.cou_id)">
								{{$t('lang.immediately')}}<br>{{$t('lang.use')}}
							</button>
							<view class="coupon_status_text" v-else-if="status == 1">{{$t('lang.have_been_used')}}</view>
							<view class="coupon_status_text" v-else-if="status == 2">{{$t('lang.have_expired')}}</view>
						</view>
					</view>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			getCouponDisplayMoney(item) {
				if (item.order_sn && item.order_coupons) {
					return item.order_coupons
				}
				return item.cou_money || 0
			},
			CommonTabs(val) {
				this.status = val
				this.couponClick(1)
...
<style>
	...
	.coupons_list .coupons_right .get_coupon_time .coupon_status_text {
		display: flex;
		align-items: center;
		justify-content: center;
		height: 100%;
	...
	.coupons_list .coupons_item.g-gay .coupons_left .collect_coupon {
		background: #ccc;
	}
	...
	.coupons_list .coupons_item.g-gay .coupons_right .coupon_status_text {
		background: #f5f5f5;
		color: #ccc;
	}
</style>
