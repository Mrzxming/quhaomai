<template>
    <view class="shop-menu">
		<view class="items">
			<view class="item" @click="$outerHref('/pagesD/shop/shopDetail/shopDetail?id='+shopId,'app')">
				<view class="info">
			...
			<view class="item" :class="{'active':bCategorySubmenu}" @click.stop="showCategory">
				...
				<view class="sub-menu" v-if="bCategorySubmenu==true">
			...
			<view class="item" @click="onChat(rz_shop_name,'',shopId)" v-else>
				<view class="info">
					<text class="iconfont icon-kefu"></text>
					<text>客服</text>
				</view>
			...
			<!-- #endif -->
			<!-- #ifdef APP-PLUS -->
			<view class="item" @click="onChat(rz_shop_name,'',shopId)">
				<view class="info">
					<text class="iconfont icon-kefu"></text>
					<text>客服</text>
				</view>
...
export default{
	...
	created(){
		...
		uni.request({
			...
			success: (res) => {
				...
				that.sKf = kf
				that.rz_shop_name = data[0].rz_shop_name
	...
	methods:{
		...
		getCatUrl(str){
			uni.navigateTo({
				url:"/pagesD/shop/shopGoods/shopGoods?ru_id="+this.shopId + "&cat_id=" + str
			})
		}
	}
}
...
<style scoped>
.shop-menu{ position: fixed; bottom: 0; width: 100%; background: #FFFFFF; border-top: 2upx solid #e3e8ee;padding-bottom: calc(env(safe-area-inset-bottom) / 2);z-index: 99999;}
