	...
	<view class="shop-list" :class="{'home-store-new': styleSel == 1}">
		<block v-if="styleSel == 0">
			<scroll-view class="scroll-view scroll-view-shop" scroll-x="true" scroll-left="0">
				<navigator :url="'/pagesD/shop/shopHome/shopHome?ru_id='+item.user_id" hover-class="none" class="scroll-view-item" v-for="(item,index) in storeList" :key="index">
		...
		<block v-else>
			<view class="floor-header-title" @click="$outerHref('/pagesD/integration/integration?type=1','app')">
				<view class="title">店铺推荐</view>
				<text class="text">{{spikeDesc}}</text>
				<view class="image"><image src="https://images.eqingqu.com/attachs/photo/202404/20240416_DD7F976FE291417DB43D4F1BB62FF1D8.png" class="img"></image></view>
			</view>
...
export default{
	...
	methods: {
		linkHref(id){
			uni.navigateTo({
				url:'/pagesD/shop/shopHome/shopHome?ru_id=' + id
			})
		},
		link(item){
			// #ifdef APP-PLUS
