    ...
    <view class="shop-signs" v-if="shopInfo">
		...
		<view class="info-nums">
			...
			<view class="info-items">
				<view class="item" @click="$outerHref('/pagesD/shop/shopGoods/shopGoods?ru_id='+shopId,'app')">全部：<text class="text">{{ shopInfo.total }}</text></view>
				<view class="item" @click="$outerHref('/pagesD/shop/shopGoods/shopGoods?ru_id='+shopId+'&type=store_new','app')">新品：<text class="text">{{ shopInfo.new }}</text></view>
				<view class="item" @click="$outerHref('/pagesD/shop/shopGoods/shopGoods?ru_id='+shopId+'&type=is_promote','app')">促销：<text class="text">{{ shopInfo.promote }}</text></view>
			</view>
		</view>
	</view>
</template>
...
<style>
	...
	.shop-signs .header{ 
		...
		position: relative; 
		/* #ifdef APP-PLUS */
		margin-top: -160upx; /* 向上延伸，覆盖搜索框下方的空白区域，让背景图从搜索框底部开始 */
		/* #endif */
	}
