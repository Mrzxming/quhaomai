	...
	<view class="shop" v-if="shopInfo[shopIndex]">
		<view class="shop-top">
			<view class="left">
				...
				<view class="info">
					<view class="name uin-ellipsis">{{ shopInfo[shopIndex].shopName }}
					<!-- <text
							v-if="shopInfo[shopIndex].shopgrade==13&&shopInfo[shopIndex].shop_name_len<15"
							style="background-color: #FE0302;color: #ffffff;border-radius: 10rpx;padding: 0rpx 12rpx;font-size: 22rpx;margin-right: 10rpx;margin-left: 10rpx;">核心供应商
						</text> -->
					</view>
					<!-- <view v-if="shopInfo[shopIndex].shopgrade==13&&shopInfo[shopIndex].shop_name_len>15"
						style="background-color: #FE0302;color: #ffffff;border-radius: 10rpx;padding: 0rpx 12rpx;font-size: 22rpx;margin-right: 10rpx;margin-left: 10rpx;width: 110rpx;">
						核心供应商</view> -->
					<view class="out" v-if="shopInfo[shopIndex].count_gaze">{{ shopInfo[shopIndex].count_gaze }}人关注
					</view>
				</view>
			</view>
			<view class="right">
				<text class="collect" v-if="shopCollect" :class="{'active':shopInfo[shopIndex].is_collect_shop == 1}"
					@click="collectHandle(shopInfo[shopIndex].ru_id)">
		...
		<view class="shop-score" v-if="shopScore && shopInfo[shopIndex].shopgrade!==13 ">
...
<script>
	...
	export default {
		...
		methods: {
			...
			linkHref(ru_id) {
				uni.redirectTo({
					url: '/pagesD/shop/shopHome/shopHome?ru_id=' + ru_id
				})
			}
		},
		watch: {
...
<style>
	...
	.shop-top .left .info .name {
		width: 100%;
		font-weight: 700;
		line-height: 1.6;
	}

	.shop-top .left .info .out {
		color: #999999;
		line-height: 1.6;
	}
