			...
			<view class="shopping_menu flex_box">
				<view class="shopping_menu_left flex_box jc_sa ai_center">
					<view class="item" :class="{'active':sort == 'sort'}" @click="filterHandle('hot')">
						<text class="size_24 weight_700 mr10">{{$t('lang.hot_alt')}}</text>
						<text class="iconfont color_ccc size_24"
							:class="[order == 'DESC' && sort == 'sort' ? 'icon-arrow-up' : 'icon-arrow-down']"></text>
					</view>

					<view class="item" :class="{'active':regionData.city.id > 0}" @click="filterHandle('region')">
						<text class="size_24 weight_700 mr10">{{$t('lang.region')}}</text>
						<text class="iconfont color_ccc size_24 icon-arrow-down"></text>
					</view>
					<view class="item" :class="{'active':sort == 'distance'}" @click="filterHandle('distance')"
						v-if="isWeixin">
						<text class="size_24 weight_700 mr10">{{$t('lang.distance')}}</text>
						<text class="iconfont color_ccc size_24"
							:class="[order == 'DESC' && sort == 'distance' ? 'icon-arrow-up' : 'icon-arrow-down']"></text>
					</view>
				</view>
				<view class="shopping_menu_right flex_box ai_center">
					<view class="search_container flex_box ai_center">
						<input class="search_input" type="text" :placeholder="$t('lang.search_the_store')"
							v-model="keyword" />
						<view class="search_btn" @click="searchTheStore"><text class="iconfont icon-home-search"></text>
						</view>
					</view>
				</view>
			</view>
					...
					<view class="scroll_view_item" :class="{'active':cat_id == item.cat_id}" v-for="(item, index) in shopCatList" :key="index"
					 @click="shopCatClick(item.cat_id)">
						<view class="icon">
							...
							<image src="https://shhai2.oss-cn-shanghai.aliyuncs.com/data/gallery_album/5/original_img/5_P_1705973846451.jpg" class="img" v-else></image>
						</view>
				...
				<view class="store-list" v-for="(item,index) in shopList" :key="index">
					<view class="header flex_box jc_sb ai_center">
						<view class="header_left flex_box ai_center">
							...
							<view class="g-title">
								<view class="" style="display: flex;align-items: center;justify-content: center;">
									<view class="size_30" style="width: 100%;display: flex;flex-direction: row;">
										<view class="shopname">{{ item.rz_shop_name }}</view>
										<!-- <view v-if="item.grade_name" class="hexin">
											核心</view> -->
									</view>
								</view>
								<view class="size_22 color_666">
									{{$t('lang.collection_one')}}{{ item.count_gaze }}{{$t('lang.collection_two')}}
								</view>
						...
						<view class="flex_box ai_center">
							<view class="into_shop_btn"
								@click="$outerHref('/pagesD/shop/shopHome/shopHome?ru_id='+item.user_id,'app')">
								{{$t('lang.into_shop')}}
							</view>
							<view :class="[item.is_collect_shop == 1 ? 'attention_btn_active' : 'attention_btn']"
								@click="collectHandle(item.user_id,item.is_collect_shop)"><text
									class="iconfont icon-shop-guanzhu"
									v-if="item.is_collect_shop != 1"></text>{{ item.is_collect_shop != 1 ? $t('lang.attention') : $t('lang.followed')}}
							</view>
						</view>
					</view>
					<view class="shop_pic" v-if="item.goods.length > 0">
						<scroll-view scroll-x="true" class="shop_scroll_box">
							<view class="product_list_item" v-for="(goodsItem,goodsIndex) in item.goods"
								:key="goodsIndex"
								@click="$outerHref('/pagesC/goodsDetail/goodsDetail?id='+goodsItem.goods_id,'app')">
...
<script>
	...
	import dscPopup from '@/components/uni-popup/uni-popup_1.vue';
	...
	export default {
		...
		onShareAppMessage(res) {
			return {
				...
				path: '/pagesD/integration/integration'
			}
		},
		created() {
			let that = this
		...
		methods: {
			shopListLoad(page, keyword = '') {
				...
				// setTimeout(function() {
				// 	uni.setStorageSync("jiazaiwan", true);	
				// }, 10);
				...
				uni.request({
					url: this.websiteUrl + '/api/shop/catshoplist',
					method: 'POST',
					data: {
						cat_id: this.cat_id,
						warehouse_id: 0,
			...
			searchTheStore() {
				if (this.keyword.trim().length > 0) {
					...
					uni.showToast({
						title: this.$t('lang.shop_name_not_null'),
						icon: 'none'
					})
				}
			},
		...
		onReachBottom() {
			...
			console.log(that.shopList.length)
			console.log(that.page * that.size == that.shopList.length)
			if (that.page * that.size == that.shopList.length) {
				that.loadMoreStatus = 'loading';
				that.page++
				that.shopListLoad()
