<template>
	<view class="container">
		<view class="header">
		<!-- 	<uni-nav-bar color="#333333" background-color="#FFFFFF" shadow="false" :statusBar="false" fixed="false"
				leftState="false" @click-right="rightClick"> -->
				<view class="input-view" style="width: 90%;">
					<input confirm-type="search" disabled="true" @click="searchFocus" class="input" type="text"
						:placeholder="placeholder" />
					<uni-icons type="search" size="22" color="#666666" class="right-icon"
						@click="searchFocus"></uni-icons>
				</view>
			<!-- 	<view slot="right"><text class="iconfont" @click="handleViewSwitch"
						:class="[mode === 'grid' ? 'icon-grid' : 'icon-list']"></text></view> -->
		...
		<view class="product-list-lie">
			<dsc-product-list :list="shopGoodsList" :mode="mode" :routerType="routerType"
				v-if="shopGoodsList"></dsc-product-list>
		</view>
		...
		<uni-drawer :visible="isPopupVisible" mode="right" @close="closeRightDrawer">
			<view class="con-filter-div">
				<view class="mod_list">
					<view class="item radio-switching">
						<view class="li_line">
							...
							<view class="right">
								<switch :checked="checkedSelf" @change="switchChange" />
							</view>
						</view>
					</view>
				</view>
				...
				<view class="tags_selection">
					<text :class="{'active':filter.goods_num == 1}"
						@click="handleTags('hasgoods')">{{$t('lang.just_look_stock')}}</text>
					<text :class="{'active':filter.promote == 1}"
						@click="handleTags('promote')">{{$t('lang.promotion')}}</text>
				</view>
				<view class="mod_list">
					<view class="item">
						<view class="li_line">
						...
						<view class="filterlayer_price">
							<view class="filterlayer_price_area">
								<input type="number" :placeholder="$t('lang.minimum_price')"
									class="filterlayer_price_area_input" v-model="filter.min" />
								<view class="filterlayer_price_hang"></view>
								<input type="number" :placeholder="$t('lang.top_price')"
									class="filterlayer_price_area_input" v-model="filter.max" />
							</view>
						</view>
					</view>
				</view>
				<view class="uni-list not">
					<view class="uni-list-cell" hover-class="uni-list-cell-hover">
						<view class="uni-list-cell-navigate uni-navigate-right" @click="selectBrand">
							{{$t('lang.brand')}}
							<text v-if="filter.brandResultName && filter.brandResultName.length > 0"
								class="right-con">{{ filter.brandResultName }}</text>
						</view>
					</view>
				</view>
				<view class="mod_list" style="margin-top: 20rpx;padding-bottom: 20rpx;">
					<view class="item">
						<view class="li_line">
							<text class="big">平台分类</text>
						</view>
						<view style="width: 95%;display: flex;flex-direction: row;justify-content: space-between;align-items: center;margin: 10rpx 20rpx;text-align: center;">
							<easy-select :options="classResult"  ref="easySelect3" size="mini" :value="className" @selectOne="selectOne"></easy-select>
							<view style="width: 30rpx;"></view>
							<easy-selects :options="rightResult"  ref="easySelect3" size="mini" :value="rightName" @selectOne="selectTwo"></easy-selects>
						</view>
					</view>
				</view>
			</view>
		...
		<uni-drawer :visible="isPopupBrand" mode="right" @close="closeRightDrawerBrand">
			<view class="sf_layer">
				<view class="sf_layer_sub_title">
					...
					<text v-if="filter.brandResultName && filter.brandResultName.length > 0"
						class="uni-ellipsis">{{ filter.brandResultName }}</text>
				</view>
				<scroll-view :scroll-y="true" class="uni-center center-box" :style="{height:winHeight-80 + 'px'}">
					<checkbox-group @change="onBrandResult">
						<label class="uni-list-item uni-list-cell uni-list-cell-pd" v-for="item in filter.brandResult"
							:key="item.brand_id">
							<view>
								<checkbox :value="item.brand_id" :checked="item.checked" />
							</view>
				...
				<view class="filterlayer_bottom_buttons">
					...
					<view class="filterlayer_bottom_button active" @click="confirmSelectBrand">{{$t('lang.confirm')}}
					</view>
				</view>
			</view>
		</uni-drawer>
...
<script>
	...
	export default {
		mixins: [createPageTrackMixin('shopsearch')],
		components: {
			uniNavBar,
			uniIcons,
			uniDrawer,
		...
		onShareAppMessage(res) {
			return {
				...
				path: '/pagesD/goodslist/goodslist'
			}
		},
		onPageScroll(e) {
			var that = this
			...
			uni.createSelectorQuery().select('.product-list-lie').boundingClientRect((res) => {
				...
				if (t_height <= to_scroll_top) {
					...
					if (that.page * that.size == that.shopGoodsList.length) {
						console.log("这里")
						that.page++
						that.getGoodsList()
					}
		...
		onReachBottom() {
			...
			console.log(that.shopGoodsList.length)
			console.log(that.page * that.size == that.shopGoodsList.length)
		...
		methods: {
			...
			getGoodsList(page,num) {
				...
				uni.request({
					url: this.websiteUrl + '/api/shop/shopgoodslist',
					method: 'POST',
					data: {
						keywords: this.keywords,
						store_id: this.ru_id,
					...
					success: (res) => {
						...
						if (res.data.status == 'success') {
							...
							if (this.page > 1) {
								this.shopGoodsList = this.shopGoodsList.concat(res.data.data)
							} else {
			...
			searchFocus() {
				...
				uni.navigateTo({
					url: '/pagesD/search/search?shopId=' + this.ru_id
				});
			},
			leftClick() {
...
<style>
	...
	.header .uni-navbar .input-view .right-icon {
		line-height: 1.5;
	}

	.header .uni-navbar .input-view .uni-icon {
		display: block;
	}
