		...
		<view class="slideshow_box">
			...
			<swiper class="slideshow" :autoplay="true" :interval="3000" :circular="true" @change="swiperChange">
				<swiper-item v-for="(swipeItem, swipeIndex) in detailData.goods_gallery" :key="swipeIndex">
					<image class="swiper_item_img" :src="swipeItem" mode="aspectFill"></image>
				</swiper-item>
			</swiper>
		</view>
		<!-- 轮播图 end -->
			...
			<view class="goods_item flex_box" @click="$outerHref('/pagesC/goodsDetail/goodsDetail?id='+detailData.goods_id,'app')">
				...
				<view class="goods_right flex_1 flex_box fd_column jc_sb">
					...
					<view class="price_box">
						<text class="size_32 weight_700 color_f0151b">{{ detailData.shop_price }}</text><text class="size_20 color_888 td_lt"><text class="size_18">¥</text>{{ detailData.market_price }}</text>
					</view>
				</view>
			</view>
		</view>
				...
				<template v-else>
					<view class="no_comment flex_box ai_center">
						<view class="no_commont_pic">
							...
							<image src="https://images.eqingqu.com/attachs/photo/202404/20240416_A18137536D3CE3482124A4EA80F3095A.png" v-else></image>
						</view>
...
<script>
	...
	export default {
		...
		onShareAppMessage(res){
			...
			return {
			  title: shareTitle,
			  path: `/pagesD/comment/commentList?id=${this.commentId}`
			}
		},
		computed: {
			indicator: function () {
