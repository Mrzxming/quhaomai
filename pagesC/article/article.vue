	...
	<view class="article">
		<view class="uni-tab-bar">
			<scroll-view id="tab-bar" class="uni-swiper-tab" scroll-x :scroll-left="scrollLeft">
				<view class="swiper-tab-list" :class="cat_id == 0 ? 'active' : ''" @click="handleArticle(0)">
					{{$t('lang.all_categories')}}</view>
				<view v-for="(item,index) in articleCateList" :key="index" class="swiper-tab-list"
					:class="cat_id == item.cat_id ? 'active' : ''" @click="handleArticle(item.cat_id)">{{item.cat_name}}
				</view>
			</scroll-view>
			<view class="article-box">
				<block v-if="articleList && articleList.length > 0">
					<view class="article-list" v-for="(item,index) in articleList" :key="index"
						@click="$outerHref('/pagesC/article/detail/detail?id='+item.id,'app')">
						<view class="title">{{item.title}}</view>
						<view class="imglist" v-if="item.content_img_list && item.content_img_list.length > 0">
							<view class="li" v-for="(imgList,imgIndex) in item.content_img_list">
