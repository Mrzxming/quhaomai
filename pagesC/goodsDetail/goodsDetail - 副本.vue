			...
			<x-skeleton type="banner" :loading="loading1" :configs="waterfallConfigs">
				<view class="goods-detail_content skeleton">
					...
					<view class="activity_goods skeleton"
						v-if="goodsDetail.promote_end_date > 0 && goodsDetail.promote_start_date > 0">
					...
					<view class="goods_module_wrap goods_base_info skeleton"
						v-if="(goodsDetail.promote_end_date > 0 && goodsDetail.promote_start_date > 0) && (formatDiscountsData.length > 0 || activityData.length > 0)" style="padding-bottom: 0rpx;">
						...
						<view class="activity_wrap activity_wrap_no" @click="showPopup('discounts')"
							...
							<view v-if="formatDiscountsData.length > 0" class="activity_main activity_coupon">
								...
								<view class="activity_list">
									<!-- 第一：优惠券（acttype: 0，包括包邮券）- 所有优惠券共用一个模块 -->
									...
									<!-- 第三：包邮（根据 is_shipping 或 freeShipping 判断） -->
									...
									<!-- 第四：积分可抵（acttype: 5） -->
									<block v-for="(activityItem, activityIndex) in formatDiscountsData" :key="'activity-points-activity-' + activityIndex">
									...
									<!-- 第五：赠送积分（acttype: 4） -->
								<!-- <block v-for="(activityItem, activityIndex) in formatDiscountsData" :key="'activity-give-integral-activity-' + activityIndex">
										<view class="promo-tag promo-tag-points" v-if="activityItem.acttype == 4">
											<text class="promo-tag-text">{{activityItem.value}}</text>
										</view>
								</block> -->
									
									<!-- 第六：跨店满减（暂时注释，样式已写好） -->
									<!-- <view class="promo-tag promo-tag-cross-store" v-if="goodsDetail.is_kj == 1 && goodsDetail.cross_warehouse_name != ''">
										<text class="promo-tag-text">跨店满减</text>
									</view> -->
									
									<!-- 第六：一件起批（根据 huodong_list_tip.one_daifa_tip 判断） -->
									<view class="promo-tag promo-tag-one-parcel" v-if="goodsDetail.huodong_list_tip && goodsDetail.huodong_list_tip.one_daifa_tip">
										<text class="promo-tag-text">{{$t('lang.one_parcel_free')}}</text>
									</view>
									
									<!-- 最后：满减（acttype: 1） -->
									<block v-for="(activityItem, activityIndex) in formatDiscountsData" :key="'activity-fullreduction-activity-' + activityIndex">
					...
					<view class="goods_module_wrap goods_base_info skeleton">
						<view class="goods_price"
							...
							<view style="margin-top: -16rpx;width:100%">
								...
								<view class="price-label" v-if="goodsDetail.volume_price_list.length > 0 ">
									<view class="price-one">
										<view class="price-top">
											<!-- <text v-if="couponLstMoney">新人价</text> -->
											<text class="new-person-price"
												v-if="goodsDetail.new_member_price == 1">新人价</text>
									...
									<view class="price-list" v-for="(item, index) in goodsDetail.volume_price_list">

										<currency-price :price="item.price" :color="'#000'" :size="24"
											:delSize="14"></currency-price>
										<text class="price-number"
											v-if="index == goodsDetail.volume_price_list.length-1">{{item.number}}</text>
								...
								<view class="activity_wrap activity_wrap_no" @click="showPopup('discounts')"
									...
									<view v-if="formatDiscountsData.length > 0" class="activity_main activity_coupon">
										...
										<view class="activity_list">
											...
											<!-- 第四：积分可抵（acttype: 5） -->
											<block v-for="(activityItem, activityIndex) in formatDiscountsData" :key="'activity-points-' + activityIndex">
												<view class="promo-tag promo-tag-points" v-if="activityItem.acttype == 5">
													<text class="promo-tag-text">{{goodsDetail.huodong_list_tip && goodsDetail.huodong_list_tip.integral_price_v1_tip ? goodsDetail.huodong_list_tip.integral_price_v1_tip : activityItem.label}}</text>
												</view>
											</block>
											...
											<!-- 第五：赠送积分（acttype: 4） -->
											<!-- <block v-for="(activityItem, activityIndex) in formatDiscountsData" :key="'activity-give-integral-' + activityIndex">
												<view class="promo-tag promo-tag-points" v-if="activityItem.acttype == 4">
													<text class="promo-tag-text">{{activityItem.value}}</text>
												</view>
											</block> -->
											...
											<!-- 最后：满减（acttype: 1） -->
											<block v-for="(activityItem, activityIndex) in formatDiscountsData" :key="'activity-fullreduction-' + activityIndex">
												<view class="promo-tag promo-tag-full-reduction" v-if="activityItem.acttype == 1">
													<text class="promo-tag-text">{{activityItem.value}}</text>
												</view>
											</block>
				...
				<view class="friend_discuss" v-if="friendDis.length > 0">
					...
					<uni-popups ref="discounts" type="bottom">
						<view class="pop_content coupon-popups" :style="{height: `${popHeight.hd}px`}">

							<scroll-view scroll-y="true" :style="{height: `${popHeight.bd}px`}">
								<view class="pop_header coupons_pop_header">
									<text class="pop_title">{{$t('lang.preferential_details')}}</text>

									<!-- <image class="close_img"
								src="https://images.eqingqu.com/attachs/photo/202404/20240416_91B3F995F947597318A8F13F391CFAD3.png"
								@click="closePop('discounts')"></image> -->
								</view>
								...
								{{$t('lang.promotion')}}
									...
									<view class="coupon-detail">
										<view class="coupon-popups-text">
											<currency-price class="coupon-popups-text-top" v-if="goodsDetail.shop_price_formated"
												:price="goodsDetail.shop_price_formated" :size="24" :delSize="14"></currency-price>
											<text class="coupon-popups-text-bot">{{$t('lang.buy_one_offer')}}</text>
										</view>
										<view class="coupon-popups-text-second">
											<view class="square"></view>
											<view class="coupon-popups-text-left">
												<text
													class="coupon-popups-text-left-top mb-8">¥{{goodsDetail.goods_price_original}}{{$t('lang.xone')}}</text>
												<view class="coupon-popups-text-left-bot">{{$t('lang.buy_one')}}</view>
											</view>
											<view class="coupon-popups-text-right" v-for="(discount, index) in discountInfo" :key="index">
												<text class="coupon-popups-text-h"></text>
												<view class="coupon-popups-text-right-label">
													<text class="coupon-popups-text-right-label-top mb-8">
														{{discount.price}}
													</text>
													<view class="coupon-popups-text-right-box">
														<text class="coupon-popups-text-right-label-new">{{discount.title}}</text>
														<view class="coupon-popups-text-right-label-new-text">
															<text>{{discount.intro}}</text>
														</view>
													</view>
												</view>
											</view>
										...
										<view class="annotation">
											<text>{{$t('lang.annotation')}}</text>
										</view>
									</view>
									<!-- 新人立减 -->
									<view v-if="Object.keys(couponLst).length > 0">
										<view class="coupons_pop_header coupons_pop_new_header">
											<view class="title-line"></view>
											<text class="pop_title">{{$t('lang.reduction_new_person')}}</text>
											<view class="title-line"></view>
										</view>

										<view class="new-member-coupon">
											<!-- 左侧金额区域 -->
											<view class="coupon-amount-section">
												<text class="currency-symbol">¥</text>
												<text class="amount-value">{{couponLstMoney}}</text>
											...
											<!-- 右侧文字区域 -->
											<view class="coupon-text-section">
												<text class="coupon-main-text">{{$t('lang.shop_reduction_new_person')}}</text>
												<text class="coupon-time-text">{{$t('lang.cou_user_receive_one')}}{{$t('lang.man')}}{{couponLst.cou_man}}{{$t('lang.in_commission')}}</text>
											</view>
										</view>
									</view>
									...
									<view v-if="goodsDetail.is_shipping == 1 || freeShipping == 1">
										<view class="coupons_pop_header coupons_pop_new_header">
											<view class="title-line"></view>
											<text class="pop_title">包邮</text>
											<view class="title-line"></view>
										</view>
										<view class="popup-content-item" v-if="goodsDetail.huodong_list_tip && goodsDetail.huodong_list_tip.baoyou_tip">
											<text class="popup-content-text">{{goodsDetail.huodong_list_tip.baoyou_tip}}</text>
										</view>
									</view>

									<!-- 积分可抵 -->
									<block v-for="(activityItem, activityIndex) in discountsData" :key="'points-' + activityIndex">
										<view v-if="activityItem.acttype == 5">
											<view class="coupons_pop_header coupons_pop_new_header">
												<view class="title-line"></view>
												<text class="pop_title">{{activityItem.label}}</text>
												<view class="title-line"></view>
											</view>
											<view class="popup-content-item popup-content-points">
												<text class="popup-content-text">{{goodsDetail.huodong_list_tip && goodsDetail.huodong_list_tip.integral_price_tip ? goodsDetail.huodong_list_tip.integral_price_tip : activityItem.value}}</text>
											</view>
										</view>
									</block>

									<!-- 赠送积分 -->
								<!-- <block v-for="(activityItem, activityIndex) in discountsData" :key="'give-integral-' + activityIndex">
										<view v-if="activityItem.acttype == 4">
											<view class="coupons_pop_header coupons_pop_new_header">
												<view class="title-line"></view>
												<text class="pop_title">{{activityItem.label}}</text>
												<view class="title-line"></view>
											</view>
											<view class="popup-content-item popup-content-cross-store">
												<text class="popup-content-text">{{activityItem.value}}</text>
											</view>
										</view>
								</block> -->

									<!-- 跨店满减（暂时隐藏，无数据时不显示） -->
									<!-- <view v-if="goodsDetail.is_kj == 1 && goodsDetail.cross_warehouse_name != ''">
										<view class="coupons_pop_header coupons_pop_new_header">
											<view class="title-line"></view>
											<text class="pop_title">跨店满减</text>
											<view class="title-line"></view>
										</view>
										<view class="popup-content-item popup-content-cross-store">
											<text class="popup-content-text">{{goodsDetail.cross_warehouse_name}}</text>
										</view>
									</view> -->

									<!-- 一件起批 -->
									<view v-if="isOneShipping">
										<view class="coupons_pop_header coupons_pop_new_header">
											<view class="title-line"></view>
											<text class="pop_title">{{$t('lang.one_parcel_free')}}</text>
											<view class="title-line"></view>
										</view>
										<view class="popup-content-item" v-if="goodsDetail.huodong_list_tip && goodsDetail.huodong_list_tip.one_daifa_tip">
											<text class="popup-content-text">{{goodsDetail.huodong_list_tip.one_daifa_tip}}</text>
										</view>
									</view>

									<!-- 满减 -->
									<block v-for="(activityItem, activityIndex) in discountsData" :key="'fullreduction-' + activityIndex">
										<view v-if="activityItem.acttype == 1">
											<view class="coupons_pop_header coupons_pop_new_header">
												<view class="title-line"></view>
												<text class="pop_title">{{activityItem.label}}</text>
												<view class="title-line"></view>
									...
									<!-- 满赠 -->
									<block v-for="(activityItem, activityIndex) in discountsData" :key="'gift-' + activityIndex">
										<view v-if="activityItem.acttype == 3">
											<view class="coupons_pop_header coupons_pop_new_header">
												<view class="title-line"></view>
												<text class="pop_title">{{activityItem.label}}</text>
												<view class="title-line"></view>
									...
									<!-- 优惠活动 -->
									<block v-if="goodsCouponList && goodsCouponList.length > 0">
										<view class="label_text coupon_activity"></view>
										<view class="coupons_list  ">
											<view class="coupons_pop_header coupons_pop_new_header">
											<view class="title-line"></view>
												<text class="pop_title">{{$t('lang.activity')}}</text><view class="title-line"></view>
												
											</view>

											<view class="coupons_item" v-for="(item,index) in goodsCouponList"
												:key="index">
												<view class="coupons_left">
													<!-- <view class="coupon_price" v-else>￥{{item.cou_money}}</view> -->
													<view class="coupons_left_top">
														<view class="coupon_price" v-if="item.cou_type==5">包邮券</view>
														<view v-else>
															<currency-price :price="item.cou_money"
																:size="24"></currency-price>
														</view>
														<view class="collect_coupon" v-if="item.cou_goods_name !== ''">
															<text>{{item.cou_goods_name}}</text>
														</view>
													</view>
													<view class="coupons_goods_name">
														<!-- <view class="coupon_tag1">{{$t('lang.coupon_tab_2')}}</view> -->
														<view class="coupon_tag2" v-if="item.cou_name !== ''">
															{{item.cou_name}}
														</view>
													</view>
													<view class="coupons_limited_period">
														<text>{{$t('lang.limited_period')}}{{ item.cou_start_time }}-{{ item.cou_end_time }}</text>
													</view>
												...
												<text class="coupon_tag">{{$t('lang.coupon_tab_2')}}</text>
												<text class="coupon_tag">{{item.cou_name}}</text>
											</view> -->
													<!-- <view class="coupon_title2">
												<view class="coupon_tag1">{{$t('lang.coupon_tab_2')}}</view>
												<view class="coupon_tag2">{{item.cou_name}}</view>
											</view>
											<view class="coupon_title" style="margin-top: -10rpx;">
												{{$t('lang.limit')}}{{item.shop_name}}{{$t('lang.usable')}}[{{item.cou_goods_name}}]
											</view> -->
													<view class="get_coupon_time">


														<!-- <text class="coupon_time">{{ item.cou_start_time }} -
													{{ item.cou_end_time }}</text> -->
														<button class="u-reset-button coupon_btn u-reset-disabled"
															v-if="item.cou_is_receive == 1 && $isLogin()">{{ $t('lang.receive_hove') }}</button>
														<button class="u-reset-button coupon_btn u-reset-disabled"
									...
									<!-- 赠送积分显示在最底部 -->
									<!-- <view class="list_wrap footer_points">

										<view class="pop_activity_item"
											v-for="(activityItem, activityIndex) in discountsData" :key="activityIndex">
											<block v-if="activityItem.acttype != 0 && activityItem.acttype > 0 && activityItem.acttype != 3">
	...
	export default {
		...
		computed: {
			...
			filteredCoupons: function() {
				for (let activityItem of this.formatDiscountsData) {
		...
		async loadSecondaryData() {
			...
			async getGoodsDetailById() {
				...
				if (status == 'success') {
					...
					that.$set(that, 'goodsDetail', data);
			...
			async getGoodsCoupon() {
				...
				this.$set(this, 'goodsCouponList', data);
