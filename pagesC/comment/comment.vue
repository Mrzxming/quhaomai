<template>
	<view class="comment_content">
		...
		<view class="wrap_box">
			...
			<view class="nav_list">
				<view :class="['nav_item', currNav == index ? 'curr_nav' : '', item.tag_name ? 'nav_li' : '']"
					v-for="(item, index) in commentTabs" :key="index" @click="toggleType(index)">{{item.title}}
					{{item.count}}
				</view>
			</view>
		</view>
					...
					<view class="comitem wrap_box" v-for="(item, index) in list" :key="index">
						...
						<view class="item_body add_comment" v-if="item.add_comment.comment_id">
							...
							<view class="imgs_scroll"
								v-if="item.add_comment.get_comment_img && item.add_comment.get_comment_img.length > 0">
								<block v-for="(img, imgIndex) in item.add_comment.get_comment_img" :key="imgIndex">
									<image class="com_img" mode="aspectFill" :lazy-load="true" :src="img"
										@click="previewImg(imgIndex, item.add_comment.get_comment_img)" v-if="img">
									</image>
									<image class="com_img" mode="aspectFill"
										src="https://shhai2.oss-cn-shanghai.aliyuncs.com/data/gallery_album/5/original_img/5_P_1705973846451.jpg"
										v-else></image>
		...
		<!-- 评价列表 end -->
		<view class="loading_txt" v-if="goodsCommentList.length > 0 && goodsCommentList[currNav].length >= size">
			{{ paginated[currNav] == 1 ? $t('lang.loading') : $t('lang.no_more') }}
		</view>
	</view>
</template>
