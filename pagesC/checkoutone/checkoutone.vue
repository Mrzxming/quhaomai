<template>
	<view class="container fixed-bottom-padding" v-if="checkoutDisplay">
		...
		<view class="add_title" @click="shopShow = true" v-if="isSingle == ''">
			<image src="https://shhai2.oss-cn-shanghai.aliyuncs.com/data/gallery_album/original_img/20230324_3cTCggnDNWDriTpkVukbTYeKiJqEUv.png"></image>
			<text>{{storepick}}</text>
		...
		<view class="flow-checkout-adr bottom_b" @click="checkoutAdd" v-if="stor == 0">
			<image src="https://shhai2.oss-cn-shanghai.aliyuncs.com/data/gallery_album/5/original_img/5_P_1705973792696.png" class="xunadd"></image>
					...
					<view class="product-items">
						<view class="item" v-for="(goodsItem,goodsIndex) in item.goods" :key="goodsIndex" v-if="goodsIndex < 3" @click="productLink(goodsItem)">
							<view class="product-img">
								...
								<image src="https://images.eqingqu.com/attachs/photo/202404/20240416_3B64DEA992DEA47F1AEF1C5FAAB1066B.png" class="icon" v-if="goodsItem.is_gift > 0"></image>
							</view>
						</view>
					</view>
				...
				<view class="uni-list-cell uni-list-cell-title">
					<view class="uni-list-cell-navigate uni-list-cell-vipzk">
						<view class="title">
							<view class="vip-zk">
								<view class="vip-img">
									<image src="https://images.eqingqu.com/attachs/photo/202404/20240416_050DB5336B33DA80AAC9A9A036D9FE25.png"></image>
								</view>
		...
		<view class="quanyiShow">
			<uni-popup :show="quanyiShow" v-on:hidePopup="handelClose('quanyi')">
				<view class="quanyi-top">
					<view class="icon-vip">
						<image src="https://images.eqingqu.com/attachs/photo/202404/20240416_050DB5336B33DA80AAC9A9A036D9FE25.png" class="img"></image>
					</view>
		...
		<uni-popup :show="couponsObject.couponsBase" type="bottom" mode="fixed" v-on:hidePopup="handelClose('coupon')">
			<view class="show-popup-bottom-common show-popup-coupon" style="height: 80vh;" @touchmove.stop.prevent="moveHandle">
				...
				<view class="content">
					...
					<view class="usable-coupon-money">使用优惠券{{couponsObject.couponsIdArr && couponsObject.couponsIdArr.length}}张,共抵扣<text class="uni-red">¥{{couponsObject.couponsMoney}}</text></view>
	...
	export default {
		...
		methods: {
			...
			shop_kud() {
				if (this.goods_id) {
					uni.redirectTo({
						url: '/pagesD/checkout/checkout?goods_id=' + this.goods_id + '&store_id=' + this.store_id + '&rec_id=' + this.rec_id+ '&ru_id=' + this.ru_id + '&spec_arr=' + this.spec_arr + '&num=' + this.num+ '&isSingle=' + this.isSingle + '&type=true'
					})
				}else{
					uni.redirectTo({
						url: '/pagesD/checkout/checkout?rec_id=' + this.rec_id + '&ru_ids=' + this.ru_id + '&store_ids=' + this.store_id + '&type=true'
					})
				}
			},
			//购物车列表
			...
			getChangetax(){
				uni.request({
					...
					success: ({
						...
						} else if (data.free_shipping_fee > 0) {
