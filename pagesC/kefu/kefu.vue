	...
	<view>
					...
					<view class="user-item" :class="{'user-item-admin':item.user_type == 1 || item.user_type == 'service'}" v-for="(item,index) in list" :key="index" v-if="list.length > 0">
						...
						<block v-else>
							<block v-if="item.user_type == 1 ||  item.user_type == 'service'">
								...
								<view class="left">
									<view class="name" style="display: flex; flex-direction: row; align-items: center;justify-content: flex-start">
									...
									<view class="cons-cont">
										<block v-if="!item.message">
											<view class="goods_link" @click="goodsLink(item.goods.goods_id)">
												...
												<view class="goods_link_right">
													<view class="goods_link_right_name twolist-hidden">123{{item.goods.goods_name}}</view>
													<view class="goods_link_right_footer">
		...
		<view class="kefu-bottom">
			<view class="footer">
				...
				<view class="text-input" style="margin-right: 20rpx;" v-if="isIpx==true">
					<input class="write-input" cursor-spacing="0" type="text" name="comment" v-model="comment" confirm-hold="true" confirm-type="发送" autocomplete="off" @confirm="btnSubmit">
				</view>
				<view class="text-input" style="margin-right: 20rpx;" v-else-if="isIpx==false">
					<input class="write-input" cursor-spacing="30" type="text" name="comment" v-model="comment" confirm-hold="true" confirm-type="发送" autocomplete="off" @confirm="btnSubmit">
				</view>
...
<script>
	...
	export default {
		...
		async onLoad(e){
			...
			console.log("~~~~~~~~~")
			console.log("ru_id:" + this.store_id)
...
.kefu-bottom .send-button .send-btn{ background: #fc0017; color: #FFFFFF; height: 70upx; line-height: 70upx; padding: 0 20upx; border-radius: 10upx; margin-right: 25upx; }
