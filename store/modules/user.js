...
const mutations = {
	[USER_LOGIN](state,o){
		...
		uni.setStorage({
			...
			success: (res) => {
				...
				// #ifdef H5
				// H5 从非 tab 页（如登录页）switchTab 可能不生效，用 reLaunch 确保跳转到目标页
				if (o.delta == 1) {
					uni.navigateBack({ delta: 1 });
				} else if (o.delta == 2 || o.delta == '2') {
					uni.reLaunch({ url: '/pages/middles/middles' });
	...
	[USER_LOGOUT](state,o){
		...
		const userId = userIdRaw != null && userIdRaw !== '' ? String(userIdRaw) : '';
		const homeCacheSuffix = userId || 'guest';
		...
		// 清除首页、消息、黑卡缓存（含首页按用户分 key 的壳层与列表缓存）
		try {
			uni.removeStorageSync('home_shell_cache_' + homeCacheSuffix);
			uni.removeStorageSync('home_goods_list_cache_' + homeCacheSuffix);
			uni.removeStorageSync('home_goods_list_cache_time_' + homeCacheSuffix);
			...
			uni.removeStorageSync('chat_sessions_cache_' + homeCacheSuffix);
			uni.removeStorageSync('chat_sessions_cache_time_' + homeCacheSuffix);
			console.log('[USER_LOGOUT] 已清除首页、消息、黑卡缓存');
		} catch (error) {
			console.error('[USER_LOGOUT] 清除页面缓存失败:', error);
		}
	...
	[USER_ORDER_DETAIL](state,o){
		...
		if(o.data && o.data.goods && Array.isArray(o.data.goods) && o.data.goods.length > 1){
			state.listShow = true
		}else{
...
const actions = {
	...
	setOrderDetail({ commit }, o){
		return apiUser.getOrderDetail(o).then(res=>{
	...
	setUserCoupon({ commit }, o){
		return apiUser.getUserCoupon(o).then(res=>{
			commit(USER_COUPON,res)
			return res
		})
	},
	getUserCoupon({ commit }, o){
		return apiUser.getUserCoupon(o)
	},
