	...
	[SHOP_CONFIG](state, {
		...
		if (!uni.getStorageSync('channelsLive')) {
			if (res.data.wxapp_media_id) {
				...
				//获取购物车数量
				store.dispatch('setMediaLivePlay').then(({
					data,
					status
				}) => {
					if (status == 'success') {
...
const actions = {
	...
	//发送短信
	async setMagicSendsms({
		commit
	}, o) {
		return apiCommon.getMagicSendsms(o)
	},
	//短信验证码验证
	async setSmsVerify({
		commit
	...
	setNewUserCoupons({
		...
		return apiCommon.getNewUserCoupons(o)
	},
	
	setNewUserGoods({
		commit
	}, o) {
		return apiCommon.getNewUserGoods(o)
	},
