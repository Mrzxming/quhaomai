<template>
	<view class="container ">
		...
		<!-- 新建分享：MP 下 :key 强制切换时重渲染 -->
		<!-- #ifdef MP-WEIXIN -->
		<view class="content-body" v-if="btnIndex==0" :key="'body-'+btnIndex+'-'+tabstatus">
		<!-- #endif -->
		<!-- #ifndef MP-WEIXIN -->
		...
		<!-- #endif -->
			...
			<view class="product-list-lie" v-if="!dscLoading&&tabstatus==2" style="margin: 0 32rpx;">
				<!-- 已购买 -->
				<view class="product-list-lie" v-if="buyList.length > 0">
...
<script>
	...
	export default {
		...
		computed: {
			...
			// 小程序导航胶囊定位（含 fallback 防 NaN 导致整页点击失效）
			wxNavLeftStyle() {
				const m = this.menuButtonInfo || {};
				const w = this.vw || 375;
				const left = w - (m.width || 87) - (m.left || 0);
				return {
					left: left + 'px',
					width: (m.width || 87) + 'px',
...
console.log('this.mpShareImg', this.mpShareImg)
				...
				uni.request({
					...
					success: (res) => {
						...
						if (res.data.status == 'success' && res.data.data.goodslist.length > 0) {
							...
							// #ifdef MP-WEIXIN
							const pg = getCurrentPages().pop();
							if (pg && pg.setData) pg.setData({ hotList: this.hotList, loadMoreStatus: this.loadMoreStatus });
							// #endif
							this.dscLoading = false
						}
					},
			...
			getexpertlist(page) {
				...
				uni.request({
					...
					success: (res) => {
						...
						if (res.data.status == 'success' && res.data.data && res.data.data.goodslist && res.data.data.goodslist.length > 0) {
							...
							// #ifdef MP-WEIXIN
							const pg = getCurrentPages().pop();
							if (pg && pg.setData) pg.setData({ expertList: this.expertList, loadMoreStatus: this.loadMoreStatus, dscLoading: false });
							// #endif
						} else {
							this.expertList = []
							this.loadMoreStatus = 'noMore'
							// #ifdef MP-WEIXIN
							const pg = getCurrentPages().pop();
							if (pg && pg.setData) pg.setData({ expertList: [], loadMoreStatus: 'noMore', dscLoading: false });
							// #endif
						}
						this.dscLoading = false
					},
				...
				uni.request({
					...
					success: (res) => {
						...
						if (res.data.status == 'success' && res.data.data.length > 0) {
							...
							// #ifdef MP-WEIXIN
							const pg = getCurrentPages().pop();
							if (pg && pg.setData) pg.setData({ buyList: this.buyList, loadMoreStatus: this.loadMoreStatus });
							// #endif
						}
						this.dscLoading = false
					},
				...
				uni.request({
					...
					success: (res) => {
						...
						if (res.data.status == 'success' && res.data.data.length > 0) {
							...
							// #ifdef MP-WEIXIN
							const pg = getCurrentPages().pop();
							if (pg && pg.setData) pg.setData({ cartList: this.cartList, loadMoreStatus: this.loadMoreStatus });
							// #endif
							this.dscLoading = false
						}
					},
				...
				uni.request({
					...
					success: (res) => {
						...
						if (res.data.status == 'success' && res.data.data.goodslist.length > 0) {
							...
							// #ifdef MP-WEIXIN
							const pg = getCurrentPages().pop();
							if (pg && pg.setData) pg.setData({ ownList: this.ownList, loadMoreStatus: this.loadMoreStatus });
							// #endif
							this.dscLoading = false
						}
					},
			...
			clickTab(index) {
				...
				// #ifdef MP-WEIXIN
				this.$set(this, 'tabstatus', idx);
				const pg = getCurrentPages().pop();
				if (pg && pg.setData) pg.setData({ tabstatus: idx });
				if (idx === 0) this.gethotlist(1);
				else if (idx === 1) this.getexpertlist(1);
				else if (idx === 2) this.getbuylist(1);
				else if (idx === 3) this.getcartlist(1);
				// #endif
				// #ifndef MP-WEIXIN
				this.tabstatus = idx;
				if (idx === 0) this.gethotlist(1);
			...
			clickBtn(index) {
				var idx = Number(index);
				// #ifdef MP-WEIXIN
				this.$set(this, 'btnIndex', idx);
				const pg = getCurrentPages().pop();
				if (pg && pg.setData) pg.setData({ btnIndex: idx });
				if (idx === 1) this.getownlist(1);
				// #endif
				// #ifndef MP-WEIXIN
				this.btnIndex = idx;
				if (idx === 1) this.getownlist(1);
