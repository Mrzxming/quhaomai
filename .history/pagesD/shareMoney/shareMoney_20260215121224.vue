<template>
	<view class="container" :class="{'mp-skyline-page': isMpWeixin}">
		<!-- 导航栏 start -->
		<view class="nav_bar skeleton" :style="{ backgroundColor: rgba,}">
			...
			<!-- #endif -->
			<!-- #ifdef MP-WEIXIN -->
			<view class="wx_nav_bar" :style="{ paddingTop: `${(menuButtonInfo && menuButtonInfo.top) || 0}px` }">
				<view class="wx_nav_left" :style="wxNavLeftStyle">
		...
		<!-- 导航栏 end -->
		<!-- #ifdef MP-WEIXIN -->
		<!-- 微信小程序 Skyline：使用 scroll-view 实现滚动 -->
		<scroll-view scroll-y class="mp-scroll-main" @scrolltolower="onScrollViewLower" @scroll="onScrollViewScroll" :show-scrollbar="false">
		<!-- #endif -->
		<!-- tab + 导航栏 start-->
		...
		<!-- 新建分享：:key 让小程序在 tab/btn 切换时强制重渲染，避免 data 更新视图不同步 -->
		<view class="content-body" v-if="btnIndex==0" :key="'body-'+btnIndex">
			<!-- 分享赚钱 -->
			<view class="tabbar bgc_fff" style="height: 100rpx;">
				<view v-for="(tab, index) in tabBars" :key="'top-'+index"
					:class="['tab-item', tabstatus == index? 'active' : '']" @click="clickTab(index)">
					<text>{{ tab }}</text>
				</view>
			</view>
			<!-- 各个分类列表切换 -->
			<view class="product-list-lie" v-if="!dscLoading&&tabstatus==0" style="margin: 0 32rpx;" :key="'list-0'">
				<!-- 爆品返利 -->
			...
			<view class="product-list-lie" v-if="!dscLoading&&tabstatus==1" style="" :key="'list-1'">
				<!-- 达人清单 -->
			...
			<view class="product-list-lie" v-if="!dscLoading&&tabstatus==2" style="margin: 0 32rpx;" :key="'list-2'">
				<!-- 已购买 -->
				<view class="product-list-lie" v-if="expertList.length > 0">
			...
			<view class=" product-list-lie" v-if="!dscLoading&&tabstatus==3" style="margin: 0 32rpx;" :key="'list-3'">
				<!-- 采购车 -->
		...
		<!-- 已分享 -->
		<view class="content-body1" v-if="btnIndex==1" :key="'body1-'+btnIndex">
		...
		<!-- 总收益 -->
		<view class="content-body2" v-if="btnIndex==2" :key="'body2-'+btnIndex">
			<!-- 总收益 -->
			...
			<view class="top-center">
				...
				<view class="user-money-list bgc_fff bora14 coa2 fs24" style="">
					<navigator url="/pagesB/bonus/bonus" class="money-item " hover-class="none">
	...
	<!-- 				<navigator url="/pagesB/valueCard/valueCard" class="money-item " hover-class="none">
			...
			<!-- #ifdef MP-WEIXIN -->
			<view class="top-center">
				<view style="margin-bottom: 10px;font-weight: 500; " class="pr12" @click="tosaleorder">
		...
		<view class="flex-center">
			<view class="tab-botbtn">
				<view class="item" v-for="(item,index) in botbtn" :key="'bot-'+index" @click="clickBtn(index)"
		...
		<!--小程序分享-->
		<!-- #ifdef MP-WEIXIN -->
		<view class="show-popup-shareImg" v-if="shareImgShow">
			<uni-popups :show="true" ref="shareImgShow" type="bottom" v-on:change="onSharePopupChange">
				<view class="mp-share-warp">
					<view class="title">
						<text>{{$t('lang.save_xaingce')}}</text>
						<uni-icon type="closeempty" size="30" color="#8f8f94" @click="closeSharePopup"></uni-icon>
					</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				user_id: (uni.getStorageSync('userInfo') && uni.getStorageSync('userInfo').id) || '',
				menuButtonInfo: {},
				vw: 375, // 小程序导航定位用，onLoad 里按 getWindowInfo 赋值，缺省防 NaN 导致点击失效
				...
				// #ifdef MP-WEIXIN
				isMpWeixin: true, // 首帧即用 Skyline 布局，避免刷新白屏
				// #endif
				// #ifndef MP-WEIXIN
				isMpWeixin: false,
				// #endif
		...
		computed: {
			...
			// 小程序导航胶囊定位，避免 vw/menuButtonInfo 未就绪时 NaN 导致整页点击失效
			wxNavLeftStyle() {
				const m = this.menuButtonInfo || {};
				const w = this.vw || 375;
				const left = Number(m.left) !== m.left ? w - 87 - 0 : w - (m.width || 87) - (m.left || 0);
				return {
					left: left + 'px',
					width: (m.width || 87) + 'px',
					height: (m.height || 32) + 'px',
					borderRadius: ((m.height || 32) / 2) + 'px'
				};
			}
		},
		watch: {
			...
			// 日志：确认 tabstatus/btnIndex 变更是否触发 Vue 响应式
			tabstatus(val, old) {
				console.log('[shareMoney] watch tabstatus:', old, '->', val);
			},
			btnIndex(val, old) {
				console.log('[shareMoney] watch btnIndex:', old, '->', val);
			}
		},
		mounted() {
			this.startRotation();
		...
		onLoad(e) {
			...
			if (e) {
				this.editid = e.id;
				const ts = e.tabstatus != null && e.tabstatus !== '' ? Number(e.tabstatus) : 0;
				this.$set(this, 'tabstatus', ts);
				this.geteditlist(1);
			}
			// #ifdef MP-WEIXIN
			this.menuButtonInfo = uni.getMenuButtonBoundingClientRect();
			const winInfo = (typeof uni.getWindowInfo === 'function' && uni.getWindowInfo()) || {};
			this.vw = winInfo.windowWidth || 375;
			this.winHeight = (winInfo.screenHeight || winInfo.windowHeight || 0) - 95;
			this.heightssChat = (winInfo.windowHeight || 0) * 0.4;
			// #endif
			// #ifndef MP-WEIXIN
			const sys = typeof uni.getWindowInfo === 'function' ? uni.getWindowInfo() : uni.getSystemInfoSync();
			this.winHeight = (sys.screenHeight || 0) - 95;
			this.heightssChat = (sys.windowHeight || 0) * 0.4;
			// #endif
			setTimeout(() => {
				this.$nextTick(() => {
					this.gethotlist(1)
		...
		onPageScroll(e) {
			...
			// #ifndef MP-WEIXIN
			// 非小程序：滚动触底在 onPageScroll 里判断，小程序仅用 onReachBottom 触底加载
			uni.createSelectorQuery().select('.content-body').boundingClientRect((res) => {
				const getScreenHeight = () => (typeof uni.getWindowInfo === 'function' ? uni.getWindowInfo() : uni.getSystemInfoSync()).screenHeight || 0;
				let a = { screenHeight: getScreenHeight() };
				if(res &&  res.height) {
		...
		onReachBottom() {
			// 非微信小程序用页面级 onReachBottom；微信 Skyline 用 scroll-view 的 onScrollViewLower
			// #ifndef MP-WEIXIN
			this.handleReachBottom();
			// #endif
		},
		methods: {
			// 微信小程序 Skyline：scroll-view 触底加载
			onScrollViewLower() {
				this.handleReachBottom();
			},
			// 微信小程序 Skyline：scroll-view 滚动，用于返回顶部按钮显隐
			onScrollViewScroll(e) {
				this.scrollState = (e.detail && e.detail.scrollTop > 800) ? true : false;
			},
			//下级会员
			drpTeam() {
			...
			// 微信小程序：v-if 控制挂载，关闭时直接置 false 即可从 DOM 移除，不再依赖 ref.close()
			closeSharePopup() {
				this.shareImgShow = false;
			},
			// 微信小程序：弹窗组件 change 事件（如点蒙层关闭）时同步 shareImgShow
			...
			onGoodsShare(info) {
				if (this.$isLogin()) {
					...
					this.$nextTick(() => {
						if (this.$refs.shareImgShow && typeof this.$refs.shareImgShow.open === 'function') {
...
console.log('this.mpShareImg', this.mpShareImg)
			...
			gethotlist(page) {
				...
				uni.request({
					...
					success: (res) => {
						console.log('[shareMoney] gethotlist success, tabstatus=', this.tabstatus, 'btnIndex=', this.btnIndex);
						uni.hideLoading();
						if (res.data.status == 'success' && res.data.data.goodslist.length > 0) {
							for (var i = 0; i < res.data.data.goodslist.length; i++) {
			...
			getexpertlist(page) {
				...
				uni.request({
					...
					success: (res) => {
						console.log('[shareMoney] getexpertlist success, tabstatus=', this.tabstatus, 'btnIndex=', this.btnIndex);
						uni.hideLoading();
						if (res.data.status == 'success' && res.data.data.goodslist.length > 0) {
			...
			getbuylist(page) {
				...
				uni.request({
					...
					success: (res) => {
						console.log('[shareMoney] getbuylist success, tabstatus=', this.tabstatus, 'btnIndex=', this.btnIndex);
						uni.hideLoading();
			...
			getcartlist(page) {
				...
				uni.request({
					...
					success: (res) => {
						console.log('[shareMoney] getcartlist success, tabstatus=', this.tabstatus, 'btnIndex=', this.btnIndex);
						uni.hideLoading();
						if (res.data.status == 'success' && res.data.data.length > 0) {
							const isAllEmpty = res.data.data.every(item => {
								return (!item.cou_name || item.cou_name.length === 0) && (!item
			...
			getownlist(page) {
				...
				uni.request({
					...
					success: (res) => {
						console.log('[shareMoney] getownlist success, tabstatus=', this.tabstatus, 'btnIndex=', this.btnIndex);
						uni.hideLoading();
						if (res.data.status == 'success' && res.data.data.goodslist.length > 0) {
							if (res.data.data.goodslist.length < 10) {
								this.loadMoreStatus = 'noMore'
			...
			clickTab(index) {
				...
				const before = this.tabstatus;
				console.log('[shareMoney] clickTab run, index=', idx, 'tabstatus before=', before);
				this.tabstatus = idx;
				console.log('[shareMoney] clickTab done, tabstatus after=', this.tabstatus);
				// 微信小程序端：改 data 后视图常不同步，必须强制更新才能切换 tab 与内容
				this.$forceUpdate();
				if (idx === 0) {
					this.gethotlist(1);
				} else if (idx === 1) {
			...
			clickBtn(index) {
				const idx = Number(index);
				const before = this.btnIndex;
				console.log('[shareMoney] clickBtn run, index=', idx, 'btnIndex before=', before);
				this.btnIndex = idx;
				console.log('[shareMoney] clickBtn done, btnIndex after=', this.btnIndex);
				// 微信小程序端：改 data 后视图常不同步，必须强制更新才能切换 tab 与内容
				this.$forceUpdate();
				if (idx === 1) {
					this.getownlist(1);
			...
			// 返回
			toBack() {
				console.log('[shareMoney] toBack click');
				uni.navigateBack()
			},
			toQues() {
...
<style lang="scss">
	...
	/* 微信小程序 Skyline：整页 flex 布局，scroll-view 占满剩余空间；仅 MP 有对应 class，他端不生效 */
	.mp-skyline-page {
		height: 100vh;
		display: flex;
	...
	.mp-skyline-page .mp-scroll-main {
		flex: 1;
		height: 0;
	}
