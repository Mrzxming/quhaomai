						...
						<block v-if="rankCurrent === 0">
							<block v-if="bargainDetailData.bargain_ranking != ''">
								<view class="item" v-for="(item, index) in bargainDetailData.bargain_ranking"
									...
									<view class="icon">
										<image src="https://images.eqingqu.com/attachs/photo/202404/20240416_5ED008A1BA5E7B8ED02E959D6AFEE7FD.png" mode="widthFix" v-if="item.rank===1">
										</image>
										<image src="https://images.eqingqu.com/attachs/photo/202404/20240416_411D0DECFB922EAB9B55243F4D723C84.png" mode="widthFix" v-if="item.rank===2">
										</image>
										<image src="https://images.eqingqu.com/attachs/photo/202404/20240416_BCDCFEF84C54A78651267224B77AAAD1.png" mode="widthFix" v-if="item.rank===3">
										</image>
									</view>
									<view class="left">
		...
		<uni-popup :show="bidShow" type="middle" v-on:hidePopup="handleClose('bid')">
			<view class="bargain-friends">
				...
				<view class="cont">{{$t('lang.bargain_propmt_5')}}<text class="uni-red"
						v-if="bargainBidData.subtract_price">¥{{bargainBidData.subtract_price}}</text>{{$t('lang.yuan')}}
				</view>
		...
		methods: {
			...
			async onBuyClicked() {
				...
				this.$store.dispatch('setBargainBuy', {
					...
					if (res.data.error == 0) {
						uni.navigateTo({
							url: '/pagesD/checkout/checkout?rec_type=8&type_id=' + res.data.bs_id
						})
					} else {
						uni.showToast({
							title: res.data.msg,
			...
			appShare() {
				let shareInfo = {
					href: `${this.$websiteUrl}bargain/detail/${this.id}?bs_id=${this.goodsInfo.bs_id}&parent_id=${uni.getStorageSync("user_id")}&splatform=APP`,
					title: this.goodsInfo.goods_name,
					summary: this.goodsInfo.bargain_desc ? this.goodsInfo.bargain_desc : this.goodsInfo.goods_brief,
					imageUrl: this.goodsInfo.goods_thumb
				};
			...
			onGoodsShare() {
				if (this.$isLogin()) {
					...
					o = {
						...
						extension_code: 'bargain',
						title: this.goodsInfo.goods_name,
						code_url: `${this.$websiteUrl}bargain/detail/${this.id}?bs_id=${this.goodsInfo.bs_id}&parent_id=${uni.getStorageSync("user_id")}&splatform=APP`,
						thumb: this.gallerylList[0],
					}
					// #endif
