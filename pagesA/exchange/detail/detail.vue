		...
		methods: {
			...
			async onExchange(){
				...
				this.$store.dispatch('setExchangeBuy',{
					...
					if(data.error == 0){
						uni.navigateTo({
							url:'/pagesD/checkout/checkout?rec_type=4'
						})
					}else{
						uni.showToast({
							icon:'none',
			...
			appShare() {
				let shareInfo = {
					href: `${this.$websiteUrl}bargain/detail/${this.goods_id}?parent_id=${uni.getStorageSync("user_id")}&splatform=APP`,
					title:this.goodsInfo.goods_name,
			...
			onGoodsShare() {
				if (this.$isLogin()) {
					...
					o = {
						...
						platform: 'APP',
						extension_code:'exchange',
						code_url:`${this.$websiteUrl}bargain/detail/${this.goods_id}?parent_id=${uni.getStorageSync("user_id")}&splatform=APP`,
						thumb:this.gallerylList[0].img_url,
					}
					// #endif
