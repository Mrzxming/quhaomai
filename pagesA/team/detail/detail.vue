...
<script>
	...
	export default {
		...
		methods: {
			...
			onTeamClicked() {
				...
				this.$store.dispatch('setTeamBuy',{
					...
					if(res.status == 'success'){
						...
						if(data.error == 0){
							uni.navigateTo({
								url:'/pagesD/checkout/checkout?rec_type=' + data.rec_type + '&type_id=' + data.t_id + '&team_id=' + data.team_id,
								complete: () => {
									uni.hideLoading();
								}
							});
			...
			appShare() {
				let shareInfo = {
					href: `${this.$websiteUrl}team/detail?goods_id=${this.goods_id}&team_id=${this.team_id}&parent_id=${uni.getStorageSync("user_id")}&splatform=APP`,
					title: this.goodsInfo.goods_name,
					summary: this.goodsInfo.goods_brief,
					imageUrl: this.goodsInfo.goods_thumb
				};
