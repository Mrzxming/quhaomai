		...
		methods: {
			...
			async onCheckoutClicked(){
				...
				this.$store.dispatch('setPresaleBuy',{
					...
					if(data.error == 0){
						uni.navigateTo({
							url:'/pagesD/checkout/checkout?rec_type=5' + '&type_id=' + data.presale_id
						})
					}else{
						uni.showToast({
							title:data.message,
			...
			appShare() {
				let shareInfo = {
					href: `${this.$websiteUrl}auction/detail/${this.act_id}?parent_id=${uni.getStorageSync("user_id")}&splatform=APP`,
					title: this.goodsInfo.goods_name,
					summary: this.goodsInfo.goods_brief,
					imageUrl: this.goodsInfo.goods_thumb
				};
