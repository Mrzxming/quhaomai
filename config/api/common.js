...
/* 新升级 带阿里图形字段的   发送短信验证码 */
function getMagicSendsms(o){
	return new Promise((reslove, reject) => {
		request.post(webUrl + '/api/misc/sms/sendupgrade',o).then(res=>{
			console.log(res)
			if(res.status == 'success'){
			    reslove(res.data.status)
			}else{
...
function getShopConfig(){
	return new Promise((reslove, reject) => {
		request.get(webUrl + '/api/shop/config', {
...
function getAppUpdate(o){
	...
	// 新人专享优惠券接口
function getNewUserCoupons(o){
	return new Promise((reslove, reject) => {
		request.get(webUrl + '/api/newuser/get_new_user_coupons',o).then(res=>{
			console.log(res)
			reslove(res)
		})
	})
	...
	// 新人专享商品列表接口
function getNewUserGoods(o){
	return new Promise((reslove, reject) => {
		request.get(webUrl + '/api/newuser/get_new_user_coupons_goods_list',o).then(res=>{
			console.log(res)
			reslove(res)
		})
	})
