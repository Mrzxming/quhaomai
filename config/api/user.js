...
function getCallback(o) {
	return new Promise((reslove, reject) => {
		if (o && o.type === 'apple') {
			console.log('[苹果登录] 请求参数:', JSON.stringify(o, null, 2));
		}
		request.post(webUrl + '/api/oauth/callback', o).then(res => {
			if (o && o.type === 'apple') {
				console.log('[苹果登录] 后端返回:', JSON.stringify(res, null, 2));
				console.log('[苹果登录] 后端返回 data:', res && res.data);
			}
			reslove(res)
		})
	})
		...
		uni.showModal({
			...
			success: (res) => {
				if (res.confirm) {
					uni.request({
						...
						success: (res) => {
							if (res.data.status == 'success') {
								...
								reject(new Error('接口返回状态不是 success'))
							}
						},
						fail: (err) => {
							reject(err)
					...
					// 用户点击取消，reject Promise 以便前端可以取消 loading
					reject(new Error('用户取消了确认收货操作'))
				}
			},
			fail: (err) => {
				reject(err)
...
/* 获取交易快照列表 */
function getSnapshotList(o) {
	return new Promise((reslove, reject) => {
		uni.request({
			url: webUrl + '/api/order/getsnapshot_list',
			method: 'GET',
			data: o,
			header: {
				'Content-Type': 'application/json',
...
/* 获取交易快照详情 */
function getSnapshotInfo(o) {
	return new Promise((reslove, reject) => {
		uni.request({
			url: webUrl + '/api/order/getsnapshot_info',
			method: 'GET',
			data: o,
			header: {
				'Content-Type': 'application/json',
